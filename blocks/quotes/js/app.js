...
const categoriesSliderSlide = document.querySelectorAll('.categories-slider__slide');
const categoriesSliderBox = document.querySelector('.categories-slider');

//window.addEventListener('resize', slidesOffsetBefore); // orientationchange
//window.addEventListener('DOMContentLoaded', slidesOffsetBefore);

function slidesOffsetBefore() {  
	if (categoriesSliderBox && categoriesSliderSlide) {
		//categoriesSlider.params.slidesOffsetBefore = 0;
		//categoriesSlider.update();
		let activeIndex = categoriesSlider.activeIndex;
		
			console.log(`activeIndex = ${activeIndex}`);
			
			//console.log(`	boxX = ${boxX}  slideX = ${slideX}`);
			if (!activeIndex) {
				console.log(`	categoriesSlider.translate = ${categoriesSlider.translate}`);
				categoriesSlider.params.slidesOffsetBefore = -categoriesSlider.translate;
				categoriesSlider.update();
				/* setTimeout(() => {
					let boxX = categoriesSliderBox.getBoundingClientRect().x;
					let slideX = categoriesSliderSlide[0].getBoundingClientRect().x;
					let delta = boxX - slideX;
					categoriesSlider.params.slidesOffsetBefore = delta;
					categoriesSlider.update();
				...
				console.log(`	categoriesSlider.translate = ${categoriesSlider.translate}`);
				/* categoriesSlider.activeIndex = 0;
				categoriesSlider.params.slidesOffsetBefore = 0;
				categoriesSlider.translate = 0;
				categoriesSlider.update(); */
				/* categoriesSlider.params.slidesOffsetBefore = -categoriesSlider.translate;
				categoriesSlider.activeIndex = activeIndex;
				categoriesSlider.update(); */
				/* setTimeout(() => {
					let boxX = categoriesSliderBox.getBoundingClientRect().x;
					let slideX = categoriesSliderSlide[0].getBoundingClientRect().x;
