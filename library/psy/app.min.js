class PsyTestsCalculator {
    ...
    init() {
        this.handler.getResult ? (this.el = document.querySelector(".js-psy-test-body"), this.el ? (this.groups = this.el.querySelectorAll(".js-psy-test-group"), this.groups.length || this._errorMsg(" Не найден селектор .js-psy-test-group")) : this._errorMsg(" Не найден селектор .js-psy-test-body")) : this._errorMsg(" Не задана функция для возвращения результата теста ")
    }
    calc() {
        let t = 0;
        const e = {},
        ...
        for (let n = 0; n < s; n++) {
            ...
            if (!i.length) throw this._errorMsg(" Не найден селектор .js-psy-test-input"), "Некорректная HTML структура теста BecksAlarmScale";
        ...
        return e.error ? (e.errTarget = this.el.querySelector(".js-psy-test-group._error"), e) : this.handler.getResult(t)
    }
    _errorMsg(t = "") {
...
class PsyTestsManager {
    ...
    _calc() {
        ...
        this.calcBtn.remove(), this.resultContainer.innerHTML = t.html, this.resultContainer.classList.add(`psy-test__info--${t.modifier}`), this.resultContainer.classList.remove("_hidden"), this.againBtn.classList.remove("_hidden"), this.saveBtn.classList.remove("_hidden"), this.againBtn.href = location.href, this.resultContainer.scrollIntoView()
    }
    _save() {
        if (this.saveBtn.style.display = "none", !this.style) return alert("К сожалению невозможно сохранить тест. \n\nСообщите об этом владельцу сайта, пожалуйста!"), void console.log("%c Не найдены стили для сохранения ", this.errCSS);
        ...
        let s = this.el.getAttribute("data-test-file-name").replaceAll(" ", "_") ?? "Test_";
        s = `${s}_от_${this._getDate()}.html`;
        ...
        r.setAttribute("href", URL.createObjectURL(i)), r.setAttribute("download", s), r.click(), this.saveBtn.style.display = ""
    }
    _getDate() {
        let t = new Date,
            e = t.getFullYear(),
        ...
        i = i < 10 ? "0" + i : i;
        let r = t.getMinutes();
        return r = r < 10 ? "0" + r : r, `${n}_${s}_${e}__${i}_${r}`
    }
    _errorMsg(t) {
        console.log(`%c ${t} `, this.errCSS), alert("К сожалению тест сломался. \n\nСообщите об этом Инне Моисеевой, пожалуйста! \n\n telegram: @innasteplom")
    }
...
class BecksAlarmScale {
    ...
    init() {
        try {
            const t = new PsyTestsCalculator(this);
            new PsyTestsManager(t)
        } catch (t) {
            throw alert("К сожалению тест сломался. \n\nСообщите об этом Инне Моисеевой, пожалуйста! \n\n telegram: @innasteplom"), t
        }
    }
    getResult(t = 0) {
        const e = Math.floor(1e4 * t / 63) / 100,
...
class BecksHopelessnessScale {
    ...
    getResult(t = 0) {
        const e = Math.floor(1e4 * t / 20) / 100,
...
class BecksSuicidalScale {
    ...
    getResult(t = 0) {
        const e = Math.floor(1e4 * t / 38) / 100,
            ...
            n = {};
        return n.html = `Риск суицида на&nbsp;момент интервью: ${t}&nbsp;из&nbsp;38 <div class="psy-test__info-count">Пороговое значение: 6 баллов </div>` + s, n.modifier = t > 20 ? "red" : t > 12 ? "orange" : t > 6 ? "yellow" : "green", n
    }
}
window.addEventListener("DOMContentLoaded", (() => {
    ! function() {
        const t = document.querySelector(".js-burger-menu");
        if (!t) return;
        const e = t.querySelectorAll(".js-burger-menu-item-btn");
        t.addEventListener("click", (s => {
            s.target.closest(".js-burger-menu-item-btn") || (t.classList.toggle("_opened"), e.length && i(e))
        }));
        ...
        function i(t) {
            t.forEach((t => {
                t.parent.classList.remove("_opened"), t.inner.setAttribute("style", "")
            }))
        }
        n && n.scrollIntoView(!0), e.length && e.forEach((s => {
            s.parent = s.parentElement, s.inner = t.querySelector(".js-burger-menu-sub-inner"), s.list = t.querySelector(".js-burger-menu-sub-list"), s.addEventListener("click", (t => {
                const n = s.parent.classList.contains("_opened");
                i(e), n || function(t) {
                    t.parent.classList.add("_opened"), t.inner.style.height = t.list.offsetHeight + "px"
                }(s)
            }))
        }))
    }(),
    function() {
        const t = document.querySelectorAll(".js-context-menu");
        t.length && (t.forEach((t => {
            const e = t.querySelectorAll(".js-context-menu-btn");
            e.length && e.forEach((e => {
                e.addEventListener("click", (e => {
                    t.classList.toggle("_opened")
                }))
            }))
        })), document.documentElement.addEventListener("click", (t => {
            if (t.target.closest(".js-context-menu-btn")) return;
            document.querySelectorAll(".js-context-menu._opened").forEach((t => t.classList.remove("_opened")))
        }), !1))
    }(),
    function() {
        const t = document.querySelector(".psy-test"),
            e = document.querySelector(".fixed-sign-up");
        location.search.match("sign_up") && (t && (t.parentElement.style.paddingBottom = "40px"), e && e.classList.remove("_hidden"))
    }()
}));
