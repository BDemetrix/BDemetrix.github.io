...
class PsyTestsCalculator {
    ...
    _errorMsg(t = "") {
        console.log(`%c "${t}" `, this.errCSS), alert("К сожалению тест сломался. Сообщите об этом владельцу сайта, пожалуйста!")
    }
}
class PsyTestsManager {
    ...
    _save() {
        ...
        n.setAttribute("href", URL.createObjectURL(i)), n.setAttribute("download", r), n.click()
    }
    _getDate() {
        let t = new Date,
            e = t.getFullYear(),
...
class BecksDepressionScale {
    ...
    getResult(t = 0) {
        const e = {};
        return t > 29 ? (e.modifier = "red", e.html = `Тяжелая депрессия <div class="psy-test__info-count">Набранных баллов: ${t} </div>`) : t > 18 ? (e.modifier = "orange", e.html = `Выраженная депрессия (депрессия средней тяжести) <div class="psy-test__info-count">Набранных баллов: ${t} </div>`) : t > 9 ? (e.modifier = "yellow", e.html = `От легкой до&nbsp;умеренной депрессии <div class="psy-test__info-count">Набранных баллов: ${t} </div>`) : (e.modifier = "green", e.html = `Нормальное состояние <div class="psy-test__info-count">Набранных баллов: ${t}</div> `), e
    }
}