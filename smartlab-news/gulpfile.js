			...
			if (items) {
				...
				for (var i = 0; i < items.length; i++) {
					...
					if (c_filename != filename) {
						fs.appendFile(src_folder + '/scss/fonts.scss', '@include font("' + fontname + '", "' + filename + '", "' + fontWeigh + '", "normal");\r\n', cb);
					}
					c_filename = filename;
					fontWeigh = 400;
				}
...
let build = gulp.series(clean, fontsCopy, fonts_otf, gulp.parallel(html, css, cssThemes, js, favicon, images, svgSprite), fonts, gulp.parallel(fontstyle));
