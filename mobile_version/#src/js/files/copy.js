// Automatic generation on 2021-09-22 09:08:18
...
(function (a) {
	...
	a.fn.datepicker.defaults.format = "dd.mm.yyyy";
	a.fn.datepicker.defaults.language = "ru";
	a.fn.datepicker.defaults.autoclose = true;
}(jQuery));
/**
	...
	new jBox.plugin("Confirm", {
		...
		_onAttach: function (t) {
			if (!this.options.confirm) {
				var o = t.attr("onclick") ? t.attr("onclick") : t.attr("href") ? t.attr("target") ? 'window.open("' + t.attr("href") + '", "' + t.attr("target") + '");' : 'window.location.href = "' + t.attr("href") + '";' : "";
				t.prop("onclick", null).data("jBox-Confirm-submit", o)
			}
		},
		_onCreated: function () {
		...
		_onOpen: function () {
			this.submitButton.off("click.jBox-Confirm" + this.id).on("click.jBox-Confirm" + this.id, function () {
				this.options.confirm ? this.options.confirm() : eval(this.source.data("jBox-Confirm-submit")), this.options.closeOnConfirm && this.close()
			}.bind(this))
		}
	})
});
jQuery(document).ready(function () {
	new jBox.plugin("Image", {
		...
		_onAttach: function (b) {
			this.images = this.images || {};
			if (b.data("jBox-image-gallery")) {
				return;
			}
			var a = b.attr(this.options.gallery) || "default";
			!this.images[a] && (this.images[a] = []);
			this.images[a].push({
				src: b.attr(this.options.src),
				label: (b.attr(this.options.imageLabel) || "")
			});
			this.options.imageLabel == "title" && b.removeAttr("title");
			b.data("jBox-image-gallery", a);
			b.data("jBox-image-id", (this.images[a].length - 1));
		},
		_onInit: function () {
			this.currentImage = {};
			this.imageZIndex = 1;
			var b = function (d, j, g, f, e, h, c) {
				if (jQuery("#jBox-image-" + d + "-" + j).length) {
					return;
				}
				var i = jQuery("<div/>", {
					id: "jBox-image-" + d + "-" + j,
					"class": "jBox-image-container" + (e ? " jBox-image-not-found" : "") + (!f && !g ? " jBox-image-" + d + "-current" : ""),
					"data-width": h,
					"data-height": c
				}).css({
					backgroundImage: e ? "" : 'url("' + this.images[d][j].src + '")',
					backgroundSize: this.options.imageSize,
					opacity: (f ? 1 : 0),
					zIndex: (g ? 0 : this.imageZIndex++)
				}).appendTo(this.content);
				jQuery("<div/>", {
					id: "jBox-image-label-" + d + "-" + j,
					"class": "jBox-image-label" + (f ? " active" : "")
				}).html(this.images[d][j].label).click(function () {
			...
			this.checkSize = function (c) {
				if ((this.content.width() <= c.data("width")) || (this.content.height() <= c.data("height"))) {
					...
					c.css("backgroundSize", "auto");
				}
			};
			this.showImage = function (d) {
				if (d != "open") {
					var c = this.currentImage.gallery;
					var f = this.currentImage.id + (1 * (d == "prev") ? -1 : 1);
					f = f > (this.images[c].length - 1) ? 0 : (f < 0 ? (this.images[c].length - 1) : f);
				} else {
					var c = this.source.data("jBox-image-gallery");
					var f = this.source.data("jBox-image-id");
					jQuery(".jBox-image-pointer-prev, .jBox-image-pointer-next").css({
						display: (this.images[c].length > 1 ? "block" : "none")
					});
				}
				if (jQuery(".jBox-image-" + c + "-current").length) {
					jQuery(".jBox-image-" + c + "-current").removeClass("jBox-image-" + c + "-current").animate({
						opacity: 0
					}, (d == "open") ? 0 : this.options.imageFade);
				}
				this.currentImage = {
					gallery: c,
					id: f
				};
				if (jQuery("#jBox-image-" + c + "-" + f).length) {
					jQuery("#jBox-image-" + c + "-" + f).addClass("jBox-image-" + c + "-current").css({
						zIndex: this.imageZIndex++,
						opacity: 0
					}).animate({
						opacity: 1
					}, (d == "open") ? 0 : this.options.imageFade);
					this.checkSize(jQuery("#jBox-image-" + c + "-" + f));
					a(c, f);
				} else {
					this.wrapper.addClass("jBox-image-loading");
					jQuery('<img src="' + this.images[c][f].src + '"/>').each(function () {
						var g = new Image();
						g.onload = function () {
							b(c, f, false, null, false, g.width, g.height);
							this.checkSize(jQuery("#jBox-image-" + c + "-" + f));
							a(c, f);
							this.wrapper.removeClass("jBox-image-loading");
						}.bind(this);
						g.onerror = function () {
							b(c, f, false, null, true, g.width, g.height);
							a(c, f);
							this.wrapper.removeClass("jBox-image-loading");
						}.bind(this);
						g.src = this.images[c][f].src;
					}.bind(this));
				}
				if (this.imageCounter) {
					if (this.images[c].length > 1) {
						this.wrapper.addClass("jBox-image-has-counter");
						this.imageCounter.find(".jBox-image-counter-all").html(this.images[c].length);
				...
				if (this.images[c].length > 1) {
					var e = f + 1;
					e = e > (this.images[c].length - 1) ? 0 : (e < 0 ? (this.images[c].length - 1) : e);
					(!jQuery("#jBox-image-" + c + "-" + e).length) && jQuery('<img src="' + this.images[c][e].src + '"/>').each(function () {
						var g = new Image();
						g.onload = function () {
							b(c, e, true, null, false, g.width, g.height);
						}.bind(this);
						g.onerror = function () {
							b(c, e, true, null, true, g.width, g.height);
						}.bind(this);
						g.src = this.images[c][e].src;
					}.bind(this));
				}
			};
		},
		_onCreated: function () {
			this.imageLabel = jQuery("<div/>", {
				"class": "jBox-image-label-container"
			...
			if (this.options.imageCounter) {
				this.imageCounter = jQuery("<div/>", {
					"class": "jBox-image-counter-container"
				}).appendTo(this.wrapper);
				this.imageCounter.append(jQuery("<span/>", {
					"class": "jBox-image-counter-current"
				})).append(jQuery("<span/>").html(this.options.imageCounterSeparator)).append(jQuery("<span/>", {
		...
		_onOpen: function () {
			jQuery(document).on("keyup.jBox-Image-" + this.id, function (a) {
				(a.keyCode == 37) && this.showImage("prev");
				(a.keyCode == 39) && this.showImage("next");
			}.bind(this));
			this.showImage("open");
		},
		_onClose: function () {
...
jQuery(document).ready(function () {
	new jBox.plugin("Notice", {
		...
		zIndex: 12e3,
		_onInit: function () {
			this.defaultNoticePosition = jQuery.extend({}, this.options.position), this._adjustNoticePositon = function () {
				var t = jQuery(window),
					i = {
						x: t.width(),
						y: t.height()
					};
				this.options.position = jQuery.extend({}, this.defaultNoticePosition), jQuery.each(this.options.responsivePositions, function (t, o) {
					if (i.x <= t) return this.options.position = o, !1
			...
			}, this.options.content instanceof jQuery && (this.options.content = this.options.content.clone().attr("id", "")), jQuery(window).on("resize.responsivejBoxNotice-" + this.id, function (t) {
		...
		_onOpen: function () {
			this._adjustNoticePositon(), jQuery.each(jQuery(".jBox-Notice"), function (t, i) {
				if (i = jQuery(i), i.attr("id") != this.id && i.data("jBox-Notice-position") == this.options.attributes.x + "-" + this.options.attributes.y) {
					if (!this.options.stack) return void i.data("jBox").close({
						ignoreDelay: !0
					});
					var o = (i.data("jBoxNoticeMargin") ? parseInt(i.data("jBoxNoticeMargin")) : parseInt(i.css("margin-" + this.options.attributes.y))) + this.wrapper.outerHeight() + this.options.stackSpacing;
					i.data("jBoxNoticeMargin", o), i.css("margin-" + this.options.attributes.y, o)
				}
			}.bind(this))
		},
		_onCloseComplete: function () {
...
$(document).ready(function () {
	...
	if (typeof (MutationObserver) != 'undefined') {
		...
		$('.sticky_block').wrapInner('<div/>');
		$('.sticky_block > div').stick_in_parent({
			parent: '.content_plus_ad',
			spacer: '.sticky_block',
			recalc_every: 1
		});
	}
	...
	$('.hamburger').on('click', function (e) {
		...
		if (!bOpened) {
			...
			$('#' + menu).css({
				'left': offset.left + 'px',
				'top': offset.top + menuitem.outerHeight() + 'px'
			});
			$('#' + menu).show();
	...
	(function ($) {
		if ($('#admin_new_bug').length) {
			$('#admin_new_bug').on('click', function () {
				JsHttpRequest.query('POST ' + aRouter.abuse + 'bug_clean_ajax/', {
					iBugId: $(this).attr('bug-id'),
					security_ls_key: LIVESTREET_SECURITY_KEY
		...
		if ($('#admin_new_spam').length) {
			$('#admin_new_spam').on('click', function () {
				JsHttpRequest.query('POST ' + aRouter.abuse + 'spam_clean_ajax/', {
					iAbuseId: $(this).attr('abuse-id'),
					security_ls_key: LIVESTREET_SECURITY_KEY
				}, function (result, errors) {
					if (!result) {
						msgErrorBox.alert('Error', 'Please try again later');
...
(function (a) {
	a(document).ready(function (e) {
		...
		var g = 0;
		var d = null;
		var c = e('<div class="Whodidit"></div>');
		e("body").append(c);
		...
		function f(h, i) {
			...
			if (!i.data.length) {
				...
				if (i.vote) {
					c.append("<div>Лайкнули: " + i.vote.count + " чел.</div>");
					c.append("<div>Рейтинг: +" + i.vote.rating + "</div>");
		...
		e("div.comments, div.topic, div.forum-top-buttons, .forum_cmts").on("mouseover", ".total a, #vote_article_counter, a.cm_mrk", function (h) {
			...
			g = setTimeout(function () {
				...
				if (k in b) {
					...
					var l = e(d).offset();
					c.empty();
					c.append("<div>Загрузка...</div>");
					c.css("left", l.left + e(d).outerWidth());
					c.css("top", l.top + e(d).outerHeight());
					c.show();
					var i = d;
					JsHttpRequest.query("POST " + i.href.replace("comment", "comment_ajax").replace("topic", "topic_ajax").replace("article", "article_ajax") + "/?security_ls_key=" + LIVESTREET_SECURITY_KEY, {}, function (m, n) {
						if (i !== d) {
							return;
						}
						if (m && !m.bStateError) {
							b[k] = {
								data: m.data,
								vote: ("vote" in m) ? m.vote : null
							};
							f(d, b[k]);
						} else {
							c.find("div").text("Ошибка!");
						}
					});
				}
			}, 500);
		...
		return new Promise(function (resolve, reject) {
			...
			if ((sEndpoint !== sOldEndpoint) || !bSubscribe || !bMute) {
				$.ajax({
					...
					data: {
						sEndpoint: sEndpoint,
						sOldEndpoint: sOldEndpoint,
						sKey: sKey,
						sAuth: sAuth,
						bSubscribe: bSubscribe ? 1 : 0,
						security_ls_key: LIVESTREET_SECURITY_KEY
					}
				}).then(function (result) {
					try {
			...
			$.ajax({
				...
				if (result.js.bStateError) {
					...
					return resolve({
						...
						targetId: targetId,
						targetType: targetType,
						bSubscribed: result.js.bSubscribed,
						bLastSubscription: result.js.bLastSubscription
					});
				}
...
(function ($) {
	$(document).ready(function ($) {
		...
		$('body').on('click', '#subscribe-sidebar-links', function (e) {
			...
			JsHttpRequest.query('POST /friends/ajax_user_subscribe/', {
				login: sLogin,
...
(function ($) {
	$(document).ready(function ($) {
		$('body').on('click', '.thisdayHide', function () {
			...
			JsHttpRequest.query('POST /thisday/ajax/', {
				action: 'hide',
				security_ls_key: LIVESTREET_SECURITY_KEY
			}, function (result, errors) {
				if (!result) return;
...
(function (a) {
	a(document).ready(function (c) {
		var b = c("#topic_ban_dialog");

		function e(g, h) {
			var i = {
				"delete": "Удалить топик",
				offtopic: "Принять меры"
			};
			i = (h in i) ? i[h] : "Что-то творим с топиком";
			var f = new jBox("Confirm", {
				...
				confirm: function () {
					...
					var r = b.find('select[name="topic_action"]');
					q.action = r.val();
					var k = "nothing";
					var p = b.find('input[name="additional_action"]');
					p.each(function (t, s) {
						if (s.checked) {
							k = c(s).val();
						}
					...
					if (k != "nothing") {
						var o = b.find('input[name="reason"]');
						o.each(function (t, s) {
							if (s.checked) {
								q.reason = s.value;
							}
						});
						var l = b.find('input[name="reason_value"]');
						if (l.length) {
							q.reason_value = l.val();
						}
						if (k == "ban") {
							var n = b.find('input[name="time"]');
							n.each(function (t, s) {
								if (n.length > 1) {
									if (s.checked) {
										q.time = s.value;
							...
							var l = b.find('input[name="time_value"]');
							l.each(function (t, s) {
								if (l.length > 1) {
									if (s.checked) {
										q.time_value = s.value;
									}
								} else {
									q.time_value = s.value;
								}
					...
					JsHttpRequest.query("POST /topic/admin/" + q.topic_id + "/?security_ls_key=" + LIVESTREET_SECURITY_KEY, q, function (s, u) {
						...
						if (s.bStateError) {
							msgErrorBox.alert(s.sMsgTitle, s.sMsg);
						} else {
							var t = c("#content").find(".topic");
							if (q.action == "delete") {
								if (t.length > 1) {
									t.each(function (v, w) {
				...
				onOpen: function () {
					...
					if (n.find('option[value="' + h + '"]').length) {
						n.val(h);
					}
					var l = b.find('input[name="additional_action"]');
					l.each(function (p, o) {
						o.checked = false;
					});
					l[0].checked = true;
					var k = b.find('input[name="reason"]');
					k.each(function (p, o) {
						o.checked = false;
					});
					k[10].checked = true;
					var j = b.find('input[type="radio"][name="time_value"]');
					if (j.length) {
						j.each(function (p, o) {
							o.checked = false;
						});
						...
						var j = b.find('input[type="radio"][name="time"]');
						if (j.length) {
							j.each(function (p, o) {
								o.checked = false;
							});
					...
					b.on("click", function (t) {
						var o = "nothing";
						var r = b.find('input[name="additional_action"]');
						r.each(function (v, u) {
							if (u.checked) {
								o = c(u).val();
							}
						...
						var s = ["radio", "text"];
						b.find("fieldset.ban_reason input").each(function (u, v) {
							if (s.indexOf(c(v).attr("type")) != -1) {
								v.disabled = (o == "nothing");
							}
						});
						b.find("fieldset.ban_time input").each(function (u, v) {
							if (s.indexOf(c(v).attr("type")) != -1) {
								v.disabled = (o != "ban");
							}
						});
						if (o != "nothing") {
							var q = b.find('input[name="reason"]');
							q.each(function (v, u) {
								if (u.checked) {
									var w = b.find('input[name="reason_value"]');
									if (w.length) {
							...
							if (o == "ban") {
								var p = b.find('input[name="time"]');
								p.each(function (v, u) {
									if (u.checked) {
										var w = b.find('input[name="time_value"]');
										if (w.length) {
		...
		var d = new RegExp("^/topic/(delete|offtopic)/(\\d+)/", "i");
		c("#content").on("click", "a[href]", function (f) {
			if (m = c(this).attr("href").match(d)) {
				f.preventDefault();
				c.ajax({
					url: aRouter.topic + "ajax_topic_ban_dialog/",
					type: "POST",
					data: {
						topic_id: parseInt(m[2]),
						security_ls_key: LIVESTREET_SECURITY_KEY
					}
				}).then(function (g) {
					if (!g.bStateError) {
						...
						c("body").append(c(g.sHtml));
						b = c("#topic_ban_dialog");
						e(parseInt(m[2]), m[1]);
					} else {
		...
		c("#content").find("a[href]").each(function (f, g) {
			$el = c(g);
			if ($el.attr("href").match(d)) {
				$el.removeAttr("onclick").attr("title", "Принять меры.");
			}
		});
	});
})(jQuery);
(function ($) {
	function fillTopicZen(iTopicId) {
		var d = $.Deferred();
		var zen = {};
		var images = {};
		var zenDialog = new jBox("Confirm", {
			id: "topicZenDialog",
			title: "Добавление топика в ленту Yandex.ZEN",
			confirmButton: "ОК",
			cancelButton: "Отмена",
			content: '\t\t\t<label for="zen_title">Название в ленте:</label>\t\t\t<input type="text" name="title" id="zen_title" value="" />\t\t\t<label for="zen_description">Аннотация:</label>\t\t\t<textarea name="description" id="zen_description"></textarea>\t\t\t<div class="image">\t\t\t\t<div>\t\t\t\t\t<img src="" />\t\t\t\t</div>\t\t\t\t<label for="zen_image_upload">Загрузить:</label>\t\t\t\t<input type="file" />\t\t\t</div>\t\t\t<label class="clear"><input type="checkbox" name="ok" />отправить в ленту</label>\t\t\t',
			confirm: function () {
				var self = this;
				var dialog = $("#" + this.id);
				var data = {
				...
				data["zen_title"] = dialog.find("#zen_title").val();
				data["zen_annotation"] = dialog.find("#zen_description").val();
				data["zen_image"] = dialog.find("img").attr("src");
				if (typeof data["zen_image"] == "undefined") data["zen_image"] = null;
				data["zen_ok"] = dialog.find("input:checkbox").is(":checked") ? 1 : 0;
				zen["zen_ok"] = data["zen_ok"];
				JsHttpRequest.query("POST /topic/ajax_fill_zen/", data, function (result, errors) {
					if (!result) {
						msgErrorBox.alert("Error", "Please try again later")
					}
					if (result.bStateError) {
			...
			onOpen: function (a) {
				var dialog = $("#" + this.id);
				dialog.find("#zen_title").val("zen_title" in zen ? zen.zen_title : "");
				dialog.find("#zen_description").val("zen_annotation" in zen ? zen.zen_annotation : "");
				dialog.find("input:checkbox").prop("checked", "zen_ok" in zen ? parseInt(zen.zen_ok) : false);
				dialog.find("img").attr("src", "zen_image" in zen ? zen.zen_image : images.length ? images[0] : null);
				if ("zen_image" in zen && images.indexOf(zen.zen_image) == -1) images.push(zen.zen_image);
				...
				dialog.find("input:file").on("change", function (e) {
					if (this.files.length) {
						var aFile = this.files[0];
						var formData = new FormData;
						formData.append("type_add", "preview");
						formData.append("img_file", aFile);
						formData.append("preview_size", "large");
						...
						return $.ajax({
							...
							try {
								result = result.match(/top\.JsHttpRequestGlobal\.dataReady\((.+)\)/);
							...
							if (result.js.bStateError) {
								...
								var m = result.js.sText.match(/<a\s+[^\>]*?href="([^\"]+)"/i);
								if (m) {
									var url = m[1];
									if (url[0] == "/") url = document.location.origin + url;
			...
			onClose: function () {
				var dialog = $("#" + this.id);
				dialog.find("img").off("click")
			},
			onCloseComplete: function () {
				d.resolve(zen.zen_ok);
				this.destroy()
			}
		});
		JsHttpRequest.query("GET /topic/ajax_fill_zen/", {
			topicId: iTopicId,
			security_ls_key: LIVESTREET_SECURITY_KEY
		}, function (result, errors) {
			if (!result) {
			...
			if (result.bStateError) {
				...
				zen = result.aZen;
				images = result.aImages;
				zenDialog.open()
			}
		}, true);
		return d
	}
	window["zen_dialog"] = fillTopicZen;
	$(document).ready(function ($) {
		new jBox("Tooltip", {
			id: "extraTagsPopover",
			trigger: "click",
			...
			onOpen: function () {
				var self = this;
				var tid = $(this.target).attr("tid");
				JsHttpRequest.query("GET /topic/ajax_edit_extratags/", {
					topicId: tid,
					security_ls_key: LIVESTREET_SECURITY_KEY
				}, function (result, errors) {
					if (!result) return;
					if (!result.bStateError) {
						if ("aAvailableExtraTags" in result || "aAvailableForumTags" in result) {
							var html = "";
							var bNeedHr = false;
							if ("aAvailableExtraTags" in result) {
								html += "<label>Экстра-теги для топика:</label>";
								for (var i in result.aAvailableExtraTags) {
									html += '<label><input type="checkbox" name="extra_tags[' + i + ']" value="' + i + '" ' + ($.inArray(i, result.aTopicExtraTags) >= 0 ? "checked" : "") + "/>" + result.aAvailableExtraTags[i]["description"] + "</label>"
								}
								bNeedHr = true
							}
							if ("bAvailableSmm" in result) {
								if (bNeedHr) html += "<hr />";
								bNeedHr = true;
								html += '<label><input type="checkbox" name="smm" value="1" ' + (parseInt(result.bAvailableSmm) ? "checked" : "") + "/><strong>SMM pipeline</strong></label>"
							}
							if ("bAvailableNews" in result) {
								if (bNeedHr) html += "<hr />";
								bNeedHr = false;
								html += '<label style="display: inline"><input id="publish_to_smartlabnews" type="checkbox" name="news" value="1" ' + (parseInt(result.bAvailableNews) ? "checked" : "") + "/><strong>NEWS</strong></label>"
							}
							if ("bAvailableNewsRedString" in result) {
								if (bNeedHr) html += "<hr />";
								bNeedHr = !("sAvailableNewsHeader" in result);
								html += '<label style="display: inline; margin-left:15px;"><input type="checkbox" name="news_red_string" value="1" ' + (parseInt(result.bAvailableNewsRedString) ? "checked" : "") + "/><strong>⚡</strong></label>"
							}
							if ("sAvailableNewsHeader" in result) {
								if (bNeedHr) html += "<hr />";
								let display = (parseInt(result.bAvailableNews) ? "checked" : "") ? "block" : "none";
								html += '<label style="display: inline; margin: 5px 0;"><input title="Опубликовать с заголовком. Чтобы вернуть заголовок поста - оставьте пустым" style="display: ' + display + ';" id="change_header_to_smartlabnews" autocomplete="off" type="text" name="news_title_string" value="' + result.sAvailableNewsHeader + '" /></label>'
							}
							if ("aCountries" in result) {
								if (bNeedHr) html += "<hr />";
								let countries_html_options = "";
								for (country of result.aCountries) {
									if (!country.in_moderator_list) {
										continue
									}
									let country_name = country.moderator_alias || country.name;
									let selected = country.id == parseInt(result.iCountryId) ? "selected" : "";
									countries_html_options += '<option value="' + country.id + '" ' + selected + ">" + country_name + "</option>"
								}
								let display = (parseInt(result.bAvailableNews) ? "checked" : "") ? "block" : "none";
								html += '<label style="display: inline; margin: 5px 0; for="news_countries_array">' + '<select style="margin:5px 0; display: ' + display + ';" id="news_countries_array" name="country_id">' + countries_html_options + "</select>" + "</label>"
							}
							if ("aTypes" in result) {
								if (bNeedHr) html += "<hr />";
								bNeedHr = true;
								let types_html_options = "";
								for (type of result.aTypes) {
									let selected = type.id == parseInt(result.iTypeId) ? "selected" : "";
									types_html_options += '<option value="' + type.id + '" ' + selected + ">" + type.name + "</option>"
								}
								let display = (parseInt(result.bAvailableNews) ? "checked" : "") ? "block" : "none";
								html += '<label style="display: inline; margin: 5px 0; for="news_types_array">' + '<select style="margin:5px 0; display: ' + display + ';" id="news_types_array" name="type_id">' + types_html_options + "</select>" + "</label>"
							}
							if ("bAvailableForumTags" in result && result.bAvailableForumTags) {
								if (bNeedHr) html += "<hr />";
								html += "<label>Компании для топика:</label>";
								if ("aAvailableForumTags" in result) {
									for (var i in result.aAvailableForumTags) {
										html += '<label><input type="checkbox" name="forum_tags[' + i + ']" value="' + i + '" checked />' + result.aAvailableForumTags[i]["title"] + "</label>"
									}
								}
								html += '<input type="text" id="forumTagAutocomplete" />'
							}
							html += '<div style="text-align: right; margin-top:1em">';
							html += '<input id="extraTagEditSubmit" type="button" value="Применить"/>';
							html += "</div>";
							self.setContent(html);
							$("#publish_to_smartlabnews").click(function () {
								if ($("#" + self.id).find('input:checkbox[name="news"]').is(":checked")) {
									$("#change_header_to_smartlabnews").css("display", "block");
									$("#news_countries_array").css("display", "block");
									$("#news_types_array").css("display", "block")
								} else {
									$("#change_header_to_smartlabnews").css("display", "none");
									$("#news_countries_array").css("display", "none");
									$("#news_types_array").css("display", "none");
									$("#" + self.id).find('input:checkbox[name="news_red_string"]').prop("checked", false)
								}
							});
							$("#forumTagAutocomplete").autocomplete({
								...
								preventBadQueries: false,
								containerClass: "autocomplete-suggestions width-auto",
								transformResult: function (response) {
									return {
								...
								onSelect: function (suggestion) {
									$("#forumTagAutocomplete").before($('<label><input type="checkbox" name="forum_tags[' + suggestion.data + ']" value="' + suggestion.data + '" checked />' + suggestion.value + "</label>"));
									$("#forumTagAutocomplete").val("")
								}
							});
							$("#extraTagEditSubmit").on("click", function (e) {
								var aTopicExtraTags = {};
								$("#" + self.id).find('input:checkbox[name^="extra_tags"]').each(function (i, el) {
									if (el.checked) aTopicExtraTags[$(el).val()] = true
								});
								var aTopicForumTags = {};
								$("#" + self.id).find('input:checkbox[name^="forum_tags"]').each(function (i, el) {
									if (el.checked) aTopicForumTags[$(el).val()] = true
								});
								var bSmm = $("#" + self.id).find('input:checkbox[name^="smm"]').is(":checked");
								var bNews = $("#" + self.id).find('input:checkbox[name="news"]').is(":checked");
								var bNewsRedString = $("#" + self.id).find('input:checkbox[name="news_red_string"]').is(":checked");
								var sAvailableNewsHeader = $("#" + self.id).find('input:text[name="news_title_string"]').val();
								var sNewsTypeId = $("#" + self.id).find('select[name="type_id"]').val();
								var sNewsCountryId = $("#" + self.id).find('select[name="country_id"]').val();
								var topicNode = $("#content").find('.extraTagEditLink[tid="' + tid + '"]').parents(".topic");
								JsHttpRequest.query("POST /topic/ajax_edit_extratags/", {
									topicId: tid,
									aTopicExtraTags: aTopicExtraTags,
									aTopicForumTags: aTopicForumTags,
									bNews: bNews,
									bNewsRedString: bNewsRedString,
									iTypeId: sNewsTypeId,
									iCountryId: sNewsCountryId,
									sAvailableNewsHeader: encodeURIComponent(sAvailableNewsHeader),
									bSmm: bSmm,
									security_ls_key: LIVESTREET_SECURITY_KEY
								}, function (result, errors) {
									if (!result) return;
									if (!result.bStateError) {
										if ("aResultTags" in result) {
											if ($.isArray(result.aResultTags)) {
												if (topicNode.find(".admin_tags").length) {
													topicNode.find(".admin_tags .tag, .admin_tags .no_tag").remove();
													topicNode.find(".admin_tags").append($('<div class="no_tag">Нет тегов</div>'))
												}
												if (topicNode.find(".tags").length) topicNode.find(".tags").empty()
											} else {
												var block = topicNode.find(".admin_tags");
												if (block.length) {
													block.find(".tag").remove();
													block.find(".no_tag").remove();
													for (var i in result.aResultTags) {
														var newTag = $('<div class="tag"><a href="/tag/' + encodeURIComponent(i) + '/"></a> | <span class="edit" title="Заменить тег в посте">ред.</span> | <span class="delete" title="Удалить тег у поста">X</span></div>');
														newTag.find("a").text(result.aResultTags[i]);
														block.append(newTag)
													}
												} else {
													if (topicNode.find(".tags").length) {
														var block = topicNode.find(".tags");
														block.append($("<li>Ключевые слова: </li>"));
														var c = 0;
														for (var i in result.aResultTags) {
															c++
														}
														...
														for (var i in result.aResultTags) {
															ct++;
															var html = '<li><a href="/tag/' + encodeURIComponent(i) + '/"></a>' + (c != ct ? ", " : "") + "</li>";
															html.find("a").text(result.aResultTags[i]);
															block.append(html)
														}
													}
												}
										...
										if ("aResultExtraTags" in result) {
											if ($.isArray(result.aResultExtraTags)) {
												topicNode.find(".ext_tags").remove()
											} else {
												var etd = topicNode.find(".ext_tags");
												if (etd.length) {
													etd.empty()
												} else {
													etd = $('<ul class="ext_tags"></ul>');
													etd.insertBefore(topicNode.find("> hr, > .tags, > .admin_tags").first())
												}
												etd.append("<li>спецраздел: </li>");
												var c = 0;
												for (var i in result.aResultExtraTags) {
													c++
												}
												var ct = 0;
												for (var i in result.aResultExtraTags) {
													ct++;
													var html = '<li><a href="' + DIR_WEB_ROOT + "/" + result.aResultExtraTags[i].url + '/">' + result.aResultExtraTags[i].description + "</a>" + (c != ct ? ", " : "") + "</li>";
													etd.append(html)
												}
											}
										}
										if ("aResultForumTags" in result) {
											if ($.isArray(result.aResultForumTags)) {
												topicNode.find(".forum_tags").remove()
											} else {
												var etd = topicNode.find(".forum_tags");
												if (etd.length) {
													etd.empty()
												} else {
													etd = $('<ul class="forum_tags"></ul>');
													etd.insertBefore(topicNode.find("> hr, > .tags, > .admin_tags").first())
												}
												etd.append("<li>обсудить на форуме: </li>");
												var c = 0;
												for (var i in result.aResultForumTags) {
													c++
												}
												var ct = 0;
												for (var i in result.aResultForumTags) {
													ct++;
													var html = '<li><a href="' + result.aResultForumTags[i].url + '">' + result.aResultForumTags[i].title + "</a>" + (c != ct ? ", " : "") + "</li>";
													etd.append(html)
												}
											}
										}
...
(function ($) {
	...
	function switchTheme(sTheme) {
		...
		if (sTheme != 'black') $('<link/>').attr({
			'rel': 'stylesheet',
			'type': 'text/css',
			'href': '/templates/skin/smart-lab-x3/themes/' + sTheme + '.css'
		}).appendTo($('head'));
	}
...
jQuery(document).ready(function ($) {
	...
	$('#plus100link').on('click', function (e) {
		...
		$.post('/profile/ajaxplus100/?JsHttpRequest=' + (new Date()).getTime() + '-xml', {
			'sLogin': sLogin,
			security_ls_key: LIVESTREET_SECURITY_KEY
		}, function (result) {
			try {
	...
	$('#allowPostNowlink').on('click', function (e) {
		var sLogin = $(e.target).attr('login');
		$.post('/profile/ajaxallownewbie/?JsHttpRequest=' + (new Date()).getTime() + '-xml', {
			'sLogin': sLogin,
			security_ls_key: LIVESTREET_SECURITY_KEY
		}, function (result) {
			try {
...
(function (b) {
	...
	function a(f) {
		var d = b(c);
		if (f.background) {
			d.find("> img").attr("src", f.background);
			d.addClass("bg");
		} else {
			d.find("> img").remove();
			d.removeClass("bg");
		}
		d.find(".profile_avatar img").attr("src", f.avatar ? f.avatar : DIR_STATIC_SKIN + "/images/avatar_100x100.jpg");
		d.find(".profile_avatar a").attr("href", f.profile);
		d.find("#user_link").attr("href", f.profile).text(f.name);
		if (f.work) {
			d.find("#user_work").text(f.work);
		} else {
			d.find("#user_work").parent("div").remove();
		}
		if (f.city) {
			d.find("#user_city").text(f.city).attr("href", aRouter.people + "city/" + f.city + "/");
		} else {
			d.find("#user_city").parent("div").remove();
		}
		d.find("#user_subscribers").text(f.readers);
		d.find("#user_friends").text(f.friends);
		d.find("#user_rating").text(Math.round(f.rating));
		d.find("#user_skill").text(Math.round(f.skill));
		d.find("#user_registred").text(f.registred);
		d.find("#blog_link").attr("href", f.blog_url);
		d.find("#forum_link").attr("href", f.forum_url);
		if (f.is_subscribed) {
			d.find("#user_subscribe_btn").addClass("active").val("Отписаться");
		}
		var e = d.find("#user_friend_btn");
		if ((f.friendship == 1) && (f.is_friend)) {
			f.friendship = 2;
		}
		switch (f.friendship) {
			case 1:
				e.addClass("active").val("Заявка подана").attr("disabled", true);
				break;
			case 2:
				e.addClass("active").val("Удалить из друзей");
				break;
			default:
			case 4:
			case 8:
			case 16:
				e.removeClass("active").val("Дружить");
				break;
		}
		return d;
	}
	b(document).ready(function () {
		...
		var g = {};
		c = c.replace("{$DIR_STATIC_SKIN}", DIR_STATIC_SKIN);
		var f = new RegExp("^/profile/([^\\/]+)/$", "i");
		var d = new jBox("Tooltip", {
			id: "profilePopover",
			trigger: "mouseenter",
			adjustDistance: {
				top: 55,
			...
			animation: "move",
			attach: b('a[href^="/profile/"]').filter(function (h) {
			...
			onOpen: function () {
				var i = this;
				var h = null;
				if (m = b(this.target).attr("href").match(f)) {
					h = m[1];
				}
				if (b(this.target).find("img").length) {
					this.target = b(this.target).find("img");
				}
				var j = function (k) {
					this.setContent(a(k));
					b("#user_pm_btn").on("click", function () {
						if (window.mailto_func && (typeof (window.mailto_func) == "function")) {
							window.mailto_func(h);
						} else {
							window.open("/talk/add/?talk_users=" + encodeURIComponent(h), "_blank");
						}
						i.close({
							ignoreDelay: true
						});
					...
					b("#user_subscribe_btn").on("click", function () {
						...
						JsHttpRequest.query("POST /friends/ajax_user_subscribe/", {
							...
							if (!n.bStateError) {
								if (h in g) {
									g[h]["is_subscribed"] = n.bSubscribeState;
								}
								var o = b("#user_subscribe_btn");
								if (n.bSubscribeState) {
									o.addClass("active").val("Отписаться");
								} else {
									o.removeClass("active").val("Подписаться");
								}
								msgNoticeBox.alert(n.sMsgTitle, n.sMsg);
							} else {
								msgErrorBox.alert(n.sMsgTitle, n.sMsg);
							}
						}, true);
					});
					b("#user_friend_btn").on("click", function () {
						switch (g[h].friendship) {
							case 1:
							case 2:
								JsHttpRequest.query("POST /profile/ajaxfrienddelete/", {
									idUser: g[h]["id"],
									sAction: "del",
									security_ls_key: LIVESTREET_SECURITY_KEY
								}, function (n, p) {
									...
									if (n.bStateError) {
										...
										msgNoticeBox.alert(n.sMsgTitle, n.sMsg);
										var o = b("#user_friend_btn");
										o.removeClass("active").val("Добавить в друзья");
										g[h].friendship = 4;
										g[h].is_friend = false;
									}
								}, true);
							...
							case 8:
							case 16:
								window.fz("track", "subscribe mini-window-addfriend");
								JsHttpRequest.query("POST /profile/ajaxfriendadd/", {
									idUser: g[h]["id"],
									userText: "",
									security_ls_key: LIVESTREET_SECURITY_KEY
								}, function (n, p) {
									...
									if (n.bStateError) {
										...
										if (g[h].friendship == 0) {
											...
											o.addClass("active").val("Удалить из друзей");
											g[h].friendship = 2;
											g[h].is_friend = true;
										}
									}
					...
					if ("ban" in window) {
						...
						if ("is_banned" in k) {
							l.attr("title", k.is_banned).addClass("banned");
						}
						if ("is_warned" in k) {
							l.attr("title", k.is_warned).addClass("warned");
						}
						b(".profile_more_menu").find("ul").append(l);
					}
					b(".profile_more a").on("click", function () {
						b(".profile_more_menu").slideToggle("fast");
					});
					b(".profile_more_menu").on("click", "li", function () {
						var q = b(this);
						if (q.hasClass("ban")) {
							i.close();
							window.ban(k.login);
							...
							if (q.hasClass("blacklist")) {
								...
								var p = "/blacklist/ajax/";
								b.ajax({
									url: p + "?JsHttpRequest=" + (new Date()).getTime() + "-xml",
									type: "POST",
									data: o
								}).then(function (n) {
									try {
									...
									if (n.js.bStateError) {
										return msgErrorBox.alert(n.js.sMsgTitle, n.js.sMsg);
									} else {
										if (n.js.sMsg) {
											msgNoticeBox.alert(n.js.sMsgTitle, n.js.sMsg);
										}
									}
									setTimeout(function () {
										document.location.reload();
				...
				if (!(h in g)) {
					JsHttpRequest.query("POST /profile/ajaxprofile/", {
						login: h,
						security_ls_key: LIVESTREET_SECURITY_KEY
					}, function (k, l) {
						if (!k) {
						...
						if (!k.bStateError) {
							k.aProfile.friendship = parseInt(k.aProfile.friendship);
							g[h] = k.aProfile;
							j.call(i, g[h]);
						}
					}, true);
			...
			onClose: function () {
				...
				b("#user_subscribe_btn").off("click");
				b("#user_friend_btn").off("click");
				b(".profile_more a").off("click");
			},
			onCloseComplete: function () {
...
(function (c) {
	c(document).ready(function (d) {
		d("body").on("mousewheel", function (h) {
			var f = d(h.target);
			if (!(f.hasClass("pm_content") || f.parents(".pm_content").length)) {
				return;
			}
			var g = f.hasClass("pm_content") ? f.get(0) : f.parents(".pm_content").get(0);
			var i = h.originalEvent.deltaY || h.originalEvent.detail || h.originalEvent.wheelDelta;
			if (i < 0 && g.scrollTop == 0) {
				h.preventDefault();
			}
			if (i > 0 && g.scrollHeight - g.clientHeight - g.scrollTop <= 1) {
				h.preventDefault();
			}
		});
		if (!d.browser.mobile && !window.ie && (typeof (localStorage) !== "undefined") && (typeof (CURRENT_USER_ID) !== "undefined")) {
			...
			d(window).smartMessanger({
				apiUrl: aRouter.talk + "messanger",
				checkUrl: "/talks",
				urlAppendParams: {
					security_ls_key: LIVESTREET_SECURITY_KEY,
					version: (typeof (SMART_MESSANGER_VERSION) !== "undefined") ? SMART_MESSANGER_VERSION : "2.5"
				},
				userId: CURRENT_USER_ID,
				sound: (typeof (SMART_MESSANGER_SOUND) !== "undefined") ? SMART_MESSANGER_SOUND : false,
				soundPath: "/plugins/smartusers/templates/sound/bb2"
			});
			window.mailto_func = function (e) {
				d(window).smartMessanger("openTalkToUser", e);
			};
		}
	});
	...
	function a(g, f) {
		...
		this.options = c.extend({
			handle: ".pm_header",
			flashInterval: 1000,
			splitNewMessages: 60000,
			scrollTimeout: 1000
		}, g);
		this.users = f.users;
		this.thread = f.thread;
		this.draft = f.draft;
		var h = {
			blockScroll: false,
			animation: "tada",
			...
			content: '<div class="pm_content"></div><div class="pm_footer"><textarea></textarea></div>',
			overlay: false,
			onOpen: function () {
				this.wrapper.addClass("pm_chat");
				this.titleContainer.addClass("pm_header").removeClass("jBox-title");
				this.title.addClass("pm_title");
				this.content.addClass("pm_wrapper");
			}
		};
		if (!("newtalk" in g)) {
			h.animation = {
		...
		var d = new jBox("Modal", h);
		d.open();
		this.node = d.wrapper.get(0);
		this.$node = d.wrapper;
		this.header = this.$node.find(this.options.handle);
		this.content = this.$node.find(".pm_content");
		this.textbox = this.$node.find(".pm_footer > textarea");
		try {
			d.open({
				ignoreDelay: true
			});
	...
	a.prototype = {
		init: function () {
			...
			this.header.on("dblclick", function (f) {
				d.$node.toggleClass("collapsed");
				if (!(d.$node.hasClass("collapsed"))) {
					d.flash(false);
				}
				d.saveWindowState();
			});
			this.textbox.on("keydown", function (g) {
				if (g.keyCode == 13) {
					if (g.ctrlKey) {
						...
						if (this.value) {
							d.sendMessage("appendDraft", {
								talkId: d.options.talkId,
								draftId: "draft_" + Math.round((Math.random() * 1000000)),
								text: this.value
							}, true);
							d.textbox.val("");
						}
			...
			this.content.on("scroll", function (h) {
				var g = h.target.scrollTop;
				var f = h.target.scrollTop + h.target.clientHeight;
				d.maxVisibleItem = 0;
				c("div[msgid]", d.content).each(function (j) {
					var k = this.getAttribute("msgid");
					if (/^draft_/i.test(k)) {
						return;
					}
					var i = this.offsetTop + this.parentNode.offsetTop + this.parentNode.parentNode.offsetTop;
					var e = i + this.clientHeight;
					if ((i >= g) && (i < f)) {
				...
				if (d.scrollTimerId) {
					clearTimeout(d.scrollTimerId);
					d.scrollTimerId = 0;
				}
				d.scrollTimerId = setTimeout(function (i) {
					d.scrollTimerId = 0;
					if (d.thread.lastView < d.maxVisibleItem) {
						d.sendMessage("viewThread", {
							talkId: d.options.talkId,
							messageId: d.maxVisibleItem
						}, true);
					}
				}, d.options.scrollTimeout);
			});
			this.$node.on("mousedown", function (f) {
				if (!c(f.target).hasClass("jBox-draggable")) {
					d.moveWindowToForeground();
				}
				d.saveWindowState();
		...
		resetup: function (d) {
			if (this.options.talkId != d) {
				try {
					pm = c.parseJSON(localStorage["pm.talks"]);
					if (this.options.talkId in pm) {
						delete pm[this.options.talkId];
					}
					localStorage["pm.talks"] = JSON.stringify(pm);
				} catch (f) {}
				this.options.talkId = d;
		...
		halt: function (d) {
			...
			this.flash(false);
			this.textbox.off("keydown");
			this.textbox.get(0).disabled = true;
			this.textbox.val(d);
			this.textbox.css("color", "red");
		...
		close: function () {
			...
			delete this.header;
			this.$node.off("mousedown touchend click");
			c.removeData(this.$node, "smartMessangerWindow");
			var d = this.$node.data("jBox");
			if (d && d.isOpen) {
				d.close({
					ignoreDelay: true
				});
		...
		flash: function (d) {
			...
			if (this.flashIntervalId) {
				...
				this.flashIntervalId = 0;
			}
			if (d) {
				if (this.$node.hasClass("collapsed")) {
					this.flashIntervalId = setInterval(function () {
						e.$node.toggleClass("hilite");
					}, this.options.flashInterval);
				}
			} else {
		...
		moveWindowToForeground: function () {
		...
		getWindowPlace: function (f) {
			var g = this;
			var h = c("html").get(0).clientWidth;
			var e = c("html").get(0).clientHeight - this.$node.height();
			c(".pm_chat").each(function (k, m) {
				var o = c(m);
				var j = parseInt(o.css("left"));
				if (!j) {
					return;
				}
				if (parseInt(o.css("top")) + o.height() > e) {
					h = Math.min(h, j);
				}
			});
			var d = (h - this.$node.width() - 4);
			if (d < 0) {
				d = 0;
			}
			this.$node.css("left", d + "px");
			this.$node.css("top", (e - 2) + "px");
			this.$node.css("right", "auto");
			this.$node.css("bottom", "auto");
			this.$node.css("z-index", jBox.zIndexMax + 1);
		},
		loadWindowState: function () {
			var h = false;
			var d = {};
			var f = {};
			if (!("pm.talks" in localStorage)) {
				localStorage["pm.talks"] = JSON.stringify({});
			}
			try {
				f = c.parseJSON(localStorage["pm.talks"]);
				if (this.options.talkId in f) {
					d = f[this.options.talkId];
					if (("left" in d) && (d.left != "auto") && ("top" in d) && (d.top != "auto")) {
						h = true;
					}
				}
			} catch (g) {
			...
			if (!h) {
				...
				d.left = this.$node.css("left");
				if (parseInt(this.$node.css("top")) < 0) {
					this.$node.css("top", "0px");
				}
				d.top = this.$node.css("top");
				d["z-index"] = this.$node.css("z-index");
				f[this.options.talkId] = d;
				localStorage["pm.talks"] = JSON.stringify(f);
			} else {
				if ("z-index" in d) {
					this.$node.css("z-index", d["z-index"]);
				}
				if ("left" in d) {
					this.$node.css("left", d.left);
				}
				if ("top" in d) {
					this.$node.css("top", d.top);
				}
				if (("collapsed" in d) && (d.collapsed)) {
					this.$node.addClass("collapsed");
				} else {
					this.$node.removeClass("collapsed");
					if ("width" in d) {
						this.$node.css("width", d.width);
					}
					if ("height" in d) {
						this.$node.find(".pm_content").css("height", d.height);
					}
				}
			}
			jBox.zIndexMax = Math.max(jBox.zIndexMax, d["z-index"]);
			var i = c(".pm_title", this.$node);
			if ("header" in this.thread) {
				i.text(this.thread.header).attr("title", this.thread.title);
			} else {
		...
		saveWindowState: function () {
			...
			var f = {};
			try {
				f = ("pm.talks" in localStorage) ? c.parseJSON(localStorage["pm.talks"]) : {};
				if (this.options.talkId in f) {
					d = f[this.options.talkId];
				}
			} catch (g) {
				console.log("Invalid pm.talks JSON while saving window state.");
			}
			if (parseInt(this.$node.css("left")) < 0) {
				this.$node.css("left", "0px");
			}
			...
			d.top = this.$node.css("top");
			d["z-index"] = this.$node.css("z-index");
			d.collapsed = this.$node.hasClass("collapsed");
			if (d.collapsed) {
				if ("width" in d) {
					delete d.width;
				}
				...
				d.width = this.$node.width() + "px";
				d.height = this.$node.find(".pm_content").height() + "px";
			}
			f[this.options.talkId] = d;
			localStorage["pm.talks"] = JSON.stringify(f);
			this.sendMessage("refreshWindowState", this.options.talkId);
		},
		clearWindowState: function () {
			try {
				var d = ("pm.talks" in localStorage) ? c.parseJSON(localStorage["pm.talks"]) : {};
				if (this.options.talkId in d) {
					delete d[this.options.talkId];
					localStorage["pm.talks"] = JSON.stringify(d);
				}
			} catch (f) {
				console.log("Invalid pm.talks JSON while clearing window state.");
			}
		},
		scrollToMessage: function (d) {
			this.content.scrollTo('[msgid="' + d + '"]');
		},
		restoreMessageState: function () {
			var f = this;
			this.content.empty();
			var h = 0;
			if (this.thread.lastView) {
				for (var e = 0, d = this.thread.messages.length; e < d; e++) {
					if (this.thread.messages[e].id == this.thread.lastView) {
						h = e - 10 - 1;
						break;
					}
				}
			...
			var g = this.thread.messages.slice(h);
			c.each(g, function (k, j) {
				f.pushTalkMesage(j);
			});
			c.each(this.draft, function (l, j) {
				var k = {
					id: j.draftId,
					text: j.text,
					uid: f.options.userId,
					time: j.time
				};
				f.pushTalkMesage(k);
			});
			if (this.draft.length) {
				...
				this.scrollToMessage(this.thread.lastView);
			}
		},
		formatMessageTime: function (f) {
			...
			var d = new Date(f);
			var g = "";
			if ((e.getDate() == d.getDate()) && (e.getMonth() == d.getMonth()) && (e.getFullYear() == d.getFullYear())) {
				g += d.getHours() < 10 ? "0" + d.getHours() : d.getHours();
				g += ":";
				g += d.getMinutes() < 10 ? "0" + d.getMinutes() : d.getMinutes();
			} else {
				g += d.getDate() < 10 ? "0" + d.getDate() : d.getDate();
				g += ".";
				g += d.getMonth() < 10 ? "0" + d.getMonth() : d.getMonth();
				g += ".";
				g += d.getFullYear().toString().substr(2);
			}
			return g;
		},
		pushTalkMesage: function (j) {
			if (this.content[0].querySelector('[msgid="' + j.id + '"]')) {
				return;
			}
			var i = false;
			var f = this.content[0].lastChild;
			if ((f != null) && (f.getAttribute("uid") == j.uid)) {
				var k = f.firstChild;
				if (k) {
					if (k.getAttribute("class") != "msg") {
						k = k.nextSibling;
					}
					var h = k.lastChild.previousSibling;
					if ((j.time - h.getAttribute("msgtime") < this.options.splitNewMessages)) {
						var e = c(k.lastChild);
						e.before(c('<div msgid="' + j.id + '" msgtime="' + j.time + '"></div>').html(j.text));
						e.text(this.formatMessageTime(j.time));
						i = true;
					}
				}
			}
			if (!i) {
				var g = (j.uid != this.options.userId);
				var f = c('<div class="' + (g ? "in" : "out") + '" uid="' + j.uid + '"></div>');
				if (g) {
					if (j.uid in this.users) {
						var d = c('<a class="nick"></a>');
						d.attr("href", this.users[j.uid].url);
						d.attr("title", this.users[j.uid].name);
						d.append(c('<img src="' + this.users[j.uid].avatar + '">'));
						f.append(d);
						d = null;
					}
				}
				var k = c('<div class="msg"></div>');
				k.append(c('<div msgid="' + j.id + '" msgtime="' + j.time + '"></div>').html(j.text));
				k.append(c('<div class="date"></div>').text(this.formatMessageTime(j.time)));
				f.append(k);
				this.content.append(f);
			}
		},
		publicDraft: function (f, e, g) {
			var d = c('[msgid="' + f + '"]', this.content);
			if (d.length) {
				d.attr("msgid", e);
				d.attr("msgtime", g);
			}
		...
		appendNewMessage: function (d) {
			this.pushTalkMesage(d);
			this.scrollToMessage(d.id);
			this.content.trigger("scroll");
			if (this.options.userId != d.uid) {
				this.flash(true);
			}
		},
		sendMessage: function (f, g, d) {
			...
			localStorage["pm.message"] = e;
			if (d) {
				var f = c.Event("storage");
				f.key = "pm.message";
				f.oldValue = localStorage["pm.message"];
				f.newValue = JSON.stringify(h);
				c(window).trigger(f);
			}
		},
	};
	c.fn.smartMessangerWindow = function (f) {
		...
		this.each(function () {
			var i = c(this);
			var h = i.data("smartMessangerWindow");
			if (!h) {
				if (!("talkId" in f)) {
					throw "talkId should be specifed.";
				}
				if (d.length < 1) {
					throw "setup data should be specifed.";
				} else {
					if (!("users" in d[0])) {
				...
				if (!("thread" in d[0])) {
					throw "thread object should be specifed in setup data.";
				}
				h = new a(this, f, d[0]);
				i.data("smartMessangerWindow", h);
			}
			if (typeof f === "string") {
	...
	c.smartMessangerWindow = function (e, d) {
		var f = new a(e, d);
		f.$node.data("smartMessangerWindow", f);
		return f.$node;
	};
	...
	function b(e) {
		...
		this.options = c.extend({
			...
			lazyUpdateInterval: 60000,
			hiddenUpdateInterval: 300000,
			userId: 0,
			userInStorageId: ("pm.userId" in localStorage) ? parseInt(localStorage["pm.userId"]) : 0,
			urlAppendParams: {},
			sound: false,
			soundPath: ""
		}, e);
	...
	b.prototype = {
		onLoad: function () {
			console.log("Loading...");
			this.userChanged(this.options.userId, true);
			try {
				this.users = ("pm.users" in localStorage) ? c.parseJSON(localStorage["pm.users"]) : {};
			} catch (f) {
				console.log("Invalid pm.users JSON while loading.");
			}
			try {
				this.threads = ("pm.threads" in localStorage) ? c.parseJSON(localStorage["pm.threads"]) : {};
			} catch (f) {
				console.log("Invalid pm.threads JSON while loading.");
			}
			try {
				this.drafts = ("pm.drafts" in localStorage) ? c.parseJSON(localStorage["pm.drafts"]) : {};
			} catch (f) {
				console.log("Invalid pm.drafts JSON while loading.");
			}
			if ("pm.talks" in localStorage) {
				try {
					var d = c.parseJSON(localStorage["pm.talks"]);
					for (talkId in d) {
						this.openTalk(talkId);
					}
				} catch (f) {
					console.log("Invalid pm.talks data while loading.");
				}
			}
		},
		onSave: function () {
		...
		onActivityChanged: function (e) {
			if (("pm.lastUpdate" in localStorage) && e) {
				var d = (new Date(localStorage["pm.lastUpdate"])).getTime();
				if ((new Date()).getTime() - d > this.options.updateInterval) {
					return this.feedUpdate(true);
				}
			}
			this.feedUpdateLoop();
		},
		...
		halt: function (f, e) {
			...
			var g = {
				auth: "Войдите на сайт",
				unknown: "Временно недоступно",
			...
			var d = (f in g) ? g[f] : g.unknown;
			for (talkId in this.talks) {
				this.talks[talkId].smartMessangerWindow("halt", d);
			}
			this.halted = true;
		},
		sound: function () {
			if (this.options.sound) {
				if (c(".jBox-Modal").length) {
					c(".jBox-Modal").data("jBox").audio({
						url: this.options.soundPath,
						volume: 75
					});
				} else {
					var d = new jBox("Modal");
					d.audio({
						url: this.options.soundPath,
						volume: 75
					});
		...
		openTalkToUser: function (e, d) {
			this.getUserTalkQuery(e, d);
		},
		openTalk: function (d) {
			this.sendMessage("openTalk", d, true);
		},
		closeTalk: function (d) {
			this.sendMessage("closeTalk", d, true);
		},
		prepareApiUrl: function (d) {
		...
		restarter: function (f) {
			...
			var d = false;
			c.ajax({
				url: e.prepareApiUrl(e.options.apiUrl + "/fresh"),
				processData: false,
				data: JSON.stringify({}),
				type: "POST",
				...
				success: function (g, i, h) {
					if (typeof (g) == "object") {
						d = true;
						if (("security_ls_key" in g) && ("userId" in g)) {
							e.options.urlAppendParams.security_ls_key = g.security_ls_key;
							if (g.userId != e.options.userId) {
								e.userChanged(g.userId, true);
							} else {
		...
		executeQuery: function (d, f, h, g) {
			...
			c.ajax({
				...
				success: function (i, k, j) {
					if (typeof (i) == "object") {
						if ("userId" in i) {
							if (i.userId != e.options.userId) {
								return e.userChanged(i.userId, true);
							} else {
		...
		newUserTalkQuery: function (d, g, e) {
			...
			this.executeQuery(this.options.apiUrl + "/newTalk", JSON.stringify(f), function (h) {
				h.draftId = e;
		...
		getUserTalkQuery: function (e, d) {
			...
			this.executeQuery(this.options.apiUrl + "/getTalk", JSON.stringify(f), function (j) {
				if ("users" in j) {
					this.sendMessage("freshUserInfo", j.users, true);
				}
				if (("talks" in j) && !c.isEmptyObject(j.talks) && !d) {
					this.sendMessage("freshTalkInfo", j.talks, true);
					for (g in j.talks) {
						if (g in this.talks) {
					...
					for (var h in j.users) {
						...
						var g = "draft_" + Math.round((Math.random() * 1000000));
						var i = {};
						i[g] = {
							draftLogin: this.users[h]["login"],
							header: this.users[h]["name"],
							title: "Новая переписка с пользователем " + this.users[h]["name"],
							url: "/talk/add/?talk_users=" + this.users[h]["login"],
							lastView: 0,
							messages: []
						};
						this.sendMessage("freshTalkInfo", i, true);
						break;
					}
				}
			});
		...
		updateQuery: function () {
			...
			for (var e in this.threads) {
				...
				if (this.threads[e] != null) {
					g.views[e] = this.threads[e].lastView;
				}
			}
			for (var e in this.threads) {
				if (e.toString().match(/^draft_/i)) {
			...
			var h = {};
			for (var e in this.drafts) {
				if (e.toString().match(/^draft_/i)) {
					continue;
				}
				if (this.drafts[e].length) {
					for (var f = 0, d = this.drafts[e].length; f < d; f++) {
						if (!this.drafts[e][f]["sended"]) {
							if (!(e in g.messages)) {
								g.messages[e] = [];
							}
							g.messages[e].push({
								draftId: this.drafts[e][f].draftId,
								text: this.drafts[e][f].text
							});
							this.drafts[e][f]["sended"] = true;
							if (!(e in h)) {
								h[e] = {};
							}
							h[e][this.drafts[e][f].draftId] = true;
						}
					}
				}
			}
			...
			this.executeQuery(this.options.apiUrl + "/update", JSON.stringify(g), function (i) {
				...
				if ("users" in i) {
					this.sendMessage("freshUserInfo", i.users, true);
				}
				if ("talks" in i) {
					this.sendMessage("freshTalkInfo", i.talks, true);
				}
				if ("errors" in i) {
					this.sendMessage("freshTalkErrorInfo", i.errors, true);
				}
			}, function () {
				localStorage["pm.lastUpdate"] = (new Date()).toUTCString();
				this.sendMessage("feedUpdateReset", null, true);
			});
		},
		feedUpdate: function (d) {
			...
			this.sendMessage("feedUpdateReset", null);
			if (d && this.options.userId) {
				console.log("Check before Feed update");
				var h = this;
				var f = this.options.checkUrl + "/";
				var k = this.options.userId.toString();
				for (var g = 0, e = k.length; g < e; g++) {
					f += k.charAt(g) + "/";
				}
				f += "notify_" + k + ".txt?r=" + Math.random();
				c.ajax({
					url: f,
					type: "GET",
					complete: function (i, j) {
						if ((i.readyState == 4) && (i.status == 200)) {
							if (!("pm.lastTag" in localStorage) || (i.responseText != localStorage["pm.lastTag"])) {
								...
								h.sendMessage("feedUpdateReset", null, true);
							}
						} else {
							h.sendMessage("feedUpdateReset", null, true);
						}
		...
		feedUpdateLoop: function () {
			var d = this;
			var e = this.options.updateInterval;
			if (c.isEmptyObject(this.talks) || !document.active) {
				e = this.options.lazyUpdateInterval;
			}
			if (document.hidden) {
				e = this.options.hiddenUpdateInterval;
			}
			console.log("Feed update timer set to " + e);
			if (this.updateTimer) {
				clearTimeout(this.updateTimer);
			}
			this.updateTimer = setTimeout(function (f) {
				d.updateTimer = 0;
				d.feedUpdate(true);
		...
		userChanged: function (f, e) {
			if (this.options.userInStorageId != f) {
				for (var d in this.talks) {
					...
					this.talks[talkId].smartMessangerWindow("close");
					this.talks[talkId].remove();
					delete this.talks[d];
				}
				this.options.userInStorageId = f;
				this.options.userId = f;
				this.talks = {};
				this.threads = {};
				this.users = {};
				this.drafts = {};
				if (e) {
					delete localStorage["pm.talks"];
					delete localStorage["pm.threads"];
					delete localStorage["pm.drafts"];
					delete localStorage["pm.users"];
					delete localStorage["pm.lastUpdate"];
					delete localStorage["pm.lastTag"];
					localStorage["pm.userId"] = f;
				}
		...
		onMessage: function (d, l, n) {
			switch (d) {
				case "freshTalkInfo":
					...
					for (var o in l) {
						if ((o in this.threads) && (this.threads[o] != null)) {
							...
							if ("header" in l[o]) {
								this.threads[o].header = l[o].header;
							}
							if ("url" in l[o]) {
								this.threads[o].url = l[o].url;
							}
							if ("reset" in l[o]) {
								delete l[o].reset;
								delete this.threads[o].messages;
								c.extend(true, this.threads[o], l[o]);
								if (o in this.talks) {
									this.talks[o].smartMessangerWindow("restoreMessageState");
								}
							} else {
								...
								for (var m = 0, k = l[o].messages.length; m < k; m++) {
									if ((u == null) || (l[o].messages[m].id > u.id)) {
										this.threads[o].messages.push(l[o].messages[m]);
										if (o in this.talks) {
											this.talks[o].smartMessangerWindow("appendNewMessage", l[o].messages[m]);
											if (l[o].messages[m].uid != this.options.userId) {
												t = true;
											}
										}
									}
					...
					break;
				case "freshUserInfo":
					c.extend(true, this.users, l);
					if (n) {
						localStorage["pm.users"] = JSON.stringify(this.users);
					}
					break;
				case "freshDraftsInfo":
					for (var o in l) {
						if (o in this.drafts) {
							for (var q in l[o]) {
								for (var m = 0, k = this.drafts[o].length; m < k; m++) {
									if (this.drafts[o][m].draftId == q) {
										this.drafts[o].splice(m, 1);
										if (o in this.talks) {
											this.talks[o].smartMessangerWindow("publicDraft", q, l[o][q], (new Date()).getTime());
										}
										if ((o in this.threads) && (this.threads[o].lastView < l[o][q])) {
											this.threads[o].lastView = l[o][q];
										}
										break;
									}
								}
					...
					break;
				case "freshTalkErrorInfo":
					if ("add" in l) {
						for (var o in l.add) {
							var h = l.add[o];
							if (o in this.talks) {
								this.talks[o].smartMessangerWindow("halt", h);
							}
						}
					}
					break;
				case "viewThread":
					if (l.talkId in this.threads) {
						this.threads[l.talkId].lastView = l.messageId;
						if (n) {
							localStorage["pm.threads"] = JSON.stringify(this.threads);
						}
					}
				...
				case "sendDrafts":
					for (var o in l) {
						for (var q in l[o]) {
							c.each(this.drafts[o], function (e, j) {
								if (j.draftId == q) {
									this.drafts[o][e]["sended"] = true;
								}
							});
						}
					}
				...
				case "appendDraft":
					if (l.talkId in this.threads) {
						...
						this.talks[l.talkId].smartMessangerWindow("pushTalkMesage", {
							id: l.draftId,
							uid: this.options.userId,
							text: l.text,
							time: (new Date()).getTime()
						});
						this.talks[l.talkId].smartMessangerWindow("scrollToMessage", l.draftId);
						if (n) {
					...
					break;
				case "feedUpdateReset":
					this.feedUpdateLoop();
					break;
				case "refreshWindowState":
					if (l in this.talks) {
						this.talks[l].smartMessangerWindow("loadWindowState");
					}
					break;
				case "changeWindowForeground":
					for (o in this.talks) {
						this.talks[o].smartMessangerWindow("loadWindowState");
					}
					break;
				case "newTalk":
					var q = l.draftId;
					...
					if (("errors" in l) && !c.isEmptyObject(l.errors)) {
						for (var m in l.errors) {
							var p = "error_" + Math.round((Math.random() * 1000000));
							this.talks[q].smartMessangerWindow("pushTalkMesage", {
								id: p,
								uid: 0,
								text: l.errors[m],
								time: (new Date()).getTime()
						...
						if (("talks" in l) && !c.isEmptyObject(l.talks)) {
							for (var o in l.talks) {
								...
								this.talks[o].smartMessangerWindow("resetup", o);
								delete this.talks[q];
								this.drafts[o] = this.drafts[q];
								delete this.drafts[q];
								l.appends = {};
								l.appends[o] = {};
								l.appends[o][this.drafts[o][0].draftId] = 0;
								this.onMessage("freshDraftsInfo", l.appends, n);
								this.threads[o] = this.threads[q];
								delete this.threads[q];
								delete this.threads[o]["draftLogin"];
								this.onMessage("freshTalkInfo", l.talks, n);
								break;
							}
						}
					}
				...
				case "openTalk":
					...
					if (!(o in this.talks)) {
						if (!(o in this.threads) || (this.threads[o] == null)) {
							...
							if ("bNewTalk" in this) {
								f.newtalk = true;
								delete this.bNewTalk;
							}
							this.talks[o] = c.smartMessangerWindow(f, {
								users: this.users,
								thread: this.threads[o],
								draft: this.drafts[o]
							});
						}
					}
				...
				case "closeTalk":
					var o = l;
					if (o in this.talks) {
						this.talks[o].smartMessangerWindow("clearWindowState");
						this.talks[o].smartMessangerWindow("close");
						this.talks[o].remove();
					}
					if (o in this.threads) {
						var s = 0;
						c.each(this.threads[o].messages, function (e, j) {
							s = Math.max(s, j.id);
						});
						this.threads[o].lastView = s;
					}
					if (n) {
					...
					delete this.talks[o];
					delete this.threads[o];
					if (!this.drafts[o].length) {
						delete this.drafts[o];
					}
					if (n) {
						localStorage["pm.threads"] = JSON.stringify(this.threads);
						localStorage["pm.drafts"] = JSON.stringify(this.drafts);
		...
		init: function () {
			var f = this;
			var l = ("pm.version" in localStorage) ? localStorage["pm.version"] : "0";
			var e = this.options.urlAppendParams.version;
			if (parseFloat(e) > parseFloat(l)) {
				this.sendMessage("halt", "refresh", false);
				if (parseFloat(l) < 2.6) {
					var k = {
						"pm.userId": true,
						"pm.lastUpdate": true
					};
					for (var g in localStorage) {
						if (g.test(/^pm\./) && (!(g in k))) {
							delete localStorage[g];
						}
					}
				}
			...
			c(window).bind("beforeunload", function (i) {
				if (f.halted) {
					return;
				}
				c(window).unbind("beforeunload", arguments.callee);
				f.onSave();
			});
			c(window).bind("storage", function (n) {
				if (f.halted) {
					return;
				}
				if (n.key == "pm.message") {
					try {
						var i = !("originalEvent" in n);
						if (!i && window.ie && (c(window).data("smartMessangerLocalMessage") === n.newValue)) {
							c.removeData(c(window), "smartMessangerLocalMessage");
							return;
						}
						var o = c.parseJSON(i ? n.newValue : n.originalEvent.newValue);
						if (!("event" in o)) {
							throw true;
						}
						try {
							if ("data" in o) {
								f.onMessage(o.event, o.data, i);
							} else {
						...
						console.log("Unknown message:" + (("originalEvent" in n) ? n.originalEvent.newValue : n.newValue));
					}
				} else {}
			});
			var j, d;
			...
			document.active = !document.hidden;
			c(document).on(d, function (i) {
				if (f.halted) {
					return;
				}
				if (j != "hidden") {
			...
			var h = 0;
			c(document).on("scroll mouseover keydown", function (i) {
				if (f.halted) {
					return;
				}
				if (h) {
	...
	c.fn.smartMessanger = function (f) {
		...
		this.each(function () {
			var i = c(this);
			var h = i.data("smartMessanger");
			if (!h) {
				h = new b(f);
				i.data("smartMessanger", h);
			}
			if (typeof f === "string") {
				if (typeof h[f] !== "function") {
	...
	function TopicDonateDialog(iTopicId) {
		...
		var options = {
			...
			confirm: function () {
				...
				$.post(aRouter['money'] + 'donate_ajax?JsHttpRequest=' + (new Date()).getTime() + '-xml', {
					target_id: iTopicId,
					target_type: 'topic',
					amount: parseInt(dialog.wrapper.find('#donate').val()),
					security_ls_key: LIVESTREET_SECURITY_KEY
				}, function (result) {
					try {
						result = JSON.parse(result);
		...
		$.getJSON(aRouter['money'] + 'donate_ajax', {
			target_id: iTopicId,
			target_type: 'topic',
			security_ls_key: LIVESTREET_SECURITY_KEY,
			JsHttpRequest: (new Date()).getTime() + '-xml'
...
(function (c) {
	...
	function a(j) {
		var d = c(b);
		d.find(".book_thumbnail img").attr("src", j.thumbnail ? j.thumbnail : DIR_WEB_ROOT + "/plugins/books/templates/skin/default/img/nocover.png");
		d.find(".book_thumbnail a").attr("href", j.profile);
		d.find(".book_title").attr("href", j.profile).text(j.title);
		if ("title_second" in j) {
			d.find(".book_subtitle").text(j.title_second);
		} else {
			d.find(".book_subtitle").parent("div").remove();
		}
		if ("authors" in j) {
			var i = d.find(".book_authors").empty();
			var h = false;
			for (var g in j.authors) {
				if (h) {
					i.append(c(document.createTextNode(", ")));
				}
				var f = c("<a></a>").attr("href", j.authors[g]).text(g);
				i.append(f);
				h = true;
			}
		} else {
			d.find(".book_authors").remove();
		}
		d.find(".book_rating > div").css("width", Math.round(parseFloat(j.rating) * 20) + "%");
		d.find(".book_rating").attr("title", "Рейтинг: " + Math.round(parseFloat(j.rating) * 100) / 100 + ", Проголосовало: " + j.votes);
		var e = d.find(".profile_panel > div");
		e.empty();
		if ("buy_url" in j) {
			e.append(c('<a href="' + j.buy_url + '" target="_blank">Купить</a>'));
		}
		if ("buy_in_english_url" in j) {
			e.append(c('<a href="' + j.buy_in_english_url + '" target="_blank">Buy</a>'));
		}
		if ("download_url" in j) {
			e.append(c('<a href="' + j.download_url + '" target="_blank">Скачать</a>'));
		}
		if (!("reviewed" in j)) {
			if (!("wishlisted" in j)) {
				e.append(c('<a href="' + (("guest_mode" in j) ? "/login" : "/books/book_add_to_wishlist/" + j.id) + '/" target="_blank">Хочу прочитать потом</a>'));
			} else {
				e.append(c('<a href="/books/book_remove_from_wishlist/' + j.id + '/" target="_blank">Не буду читать</a>'));
			}
			e.append(c('<a href="' + (("guest_mode" in j) ? "/login" : "/review/add/for/" + j.id) + '/" target="_blank">Я ее читал</a>'));
		}
		if (d.scrollHeight > 0) {
			d.css("height", (d.height() + d.scrollHeight) + "px");
			e.css("bottom", "0");
		}
		return d;
	}
	c(document).ready(function () {
		...
		new jBox("Tooltip", {
			...
			closeOnMouseleave: true,
			animation: "move",
			attach: c('a[href^="/books/"]').filter(function (j) {
				if (c(this).attr("href").match(e) !== null) {
					return true;
				}
				var h = c(this).attr("href").match(f);
				if (h && (d.indexOf(h[1]) == -1)) {
					return true;
				}
				return false;
			}),
			...
			onOpen: function () {
				...
				var n = null;
				var k = null;
				if (m = c(this.target).attr("href").match(e)) {
					n = m[1];
				}
				if (m = c(this.target).attr("href").match(f)) {
					k = m[1];
				}
				if (c(this.target).find("img").length) {
					this.target = c(this.target).find("img");
				}
				var l = function (o) {
					this.setContent(a(o));
				};
				var i = (n != null) ? "id:" + n : "u:" + k;
				if (!(i in g)) {
					var h = {};
					if ("LIVESTREET_SECURITY_KEY" in window) {
					...
					JsHttpRequest.query("POST /books/ajaxprofile/", h, function (o, p) {
						if (!o) {
							return;
						}
						if (!o.bStateError) {
		...
		function help_func(command) {
					...
					for (var cmd in settings.commands) {
						if (cmd == command) {
							...
							data.push({
								l: cmd,
								t: t,
								s: ('syntax' in v) ? v['syntax'] : cmd,
								a: need_auth(settings.commands[cmd]),
								oc: cmd
							});
			...
			if (cmd in settings.commands) {
				...
				for (var i in settings.commands) {
					...
					var item = {
						l: i,
						t: settings.commands[i]['title'],
						c: 'Подробнее о команде ' + i,
						a: need_auth(settings.commands[i])
					};
					data.push(item);
				}
				data.push({
					l: 'HELP',
					t: 'Выводит это сообщение',
					c: 'Это вся доступная помощь \'(',
					a: false
				});
				combo.trigger({
					type: 'reset',
				...
				case 40:
					{
						if (autocomplete.box.css('display') != 'none') {
							e.preventDefault();
							autocomplete.box.trigger(jQuery.Event('keypress', {
								keyCode: e.which
							}));

						} else if (combo.css('display') != 'none') {
							e.preventDefault();
							combo.trigger(jQuery.Event('keypress', {
								keyCode: e.which
							}));
						}
						break;
	...
	$.fn.smart_console_dropdown = function (options) {
		...
		this.on('mousedown', '> div', function (e) {
			...
			if (self.attr('mouse_pressed')) {
				...
				settings.parent.trigger({
					type: 'command',
					fromPopup: self,
					message: $(this).attr('unique')
				});
				...
				case 13: // enter
					{
						...
						settings.parent.trigger({
							type: 'command',
							fromPopup: self,
							message: selected.length ? selected.attr('unique') : null
						});
						settings.parent.focus();
						break;
					}
...
var console_commands = {
	'FIND': {
		title: 'Поиск',
		variants: [{
				title: 'Поиск по финсловарю',
				start_title: 'По словарю',
				command: 'FIND F ',
				syntax: 'FIND F искомое',
				params: /^F\s(.*)$/i,
				ac: /^F\s([^@]+)$/i,
				act: 'wiki',
				url: '/search/results/?q=$1&in=articles',
				tooltip: 'Введите слово'
			},
			{
				title: 'Поиск книг',
				start_title: 'Книгу',
				command: 'FIND B ',
				syntax: 'FIND B искомое',
				params: /^B\s(.*)$/i,
				ac: /^B\s([^@]+)$/i,
				act: 'book',
				url: '/books/book_search?q=$1',
				tooltip: 'Введите название книги',
				keywords: 'искать книгу,найти книгу'
			},
			{
				title: 'Поиск людей',
				start_title: 'Людей',
				...
				params: /^\@([0-9a-z\_\-]+)$/i,
				ac: /^\@(.*)$/i,
				url: '/search/results/?q=$1&in=users',
				tooltip: 'Введите логин или имя',
				keywords: 'искать человека,найти человека'
			},
			{
				title: 'Поиск по блогам',
				start_title: 'Текст в блогах',
				command: 'FIND ',
				syntax: 'FIND искомое',
				params: /^(.*)$/,
				url: '/search/results/?q=$1&in=topics',
				tooltip: 'Введите текст',
				keywords: 'искать текст,найти текст'
			}, // ac: /^([^@]+)$/i, act: 'tag',
			{
				title: 'Поиск записей трейдера в блогах',
				command: 'FIND ',
				syntax: 'FIND искомое@login',
				params: /^(.*)$/,
				ac: /(.+)\@(.*)$/i,
				url: '/search/results/?q=$1&in=topics'
			}
	...
	'BLOG': {
		...
		variants: [{
			...
			{
				title: 'Блог Романа Андреева',
				syntax: 'BLOG @RomanAndreev',
				redirect_url: '/my/RomanAndreev/blog/',
				keywords: 'Роман Андреев'
			},
			{
				title: 'Блог пользователя',
				start_title: 'Блоги людей',
				command: 'BLOG @',
				syntax: 'BLOG @login',
				params: /^\@([0-9a-z\_\-]+)$/i,
				ac: /^\@(.*)/i,
	...
	'MSG': {
		...
		variants: [{
			...
			func: 'mailto_func',
			auth_required: true,
			keywords: 'личное сообщение,написать сообщение,сообщение написать',
			tooltip: 'Введите имя кому'
		}]
	},

	'ALL': {
		title: 'Все блоги подряд',
		command: 'ALL',
		syntax: 'ALL или BLOGS',
		redirect_url: '/allblog/',
		keywords: 'лента всех постов'
	},
	'ECO': {
		title: 'Календарь всех событий',
	...
	'WIKI': {
		...
		variants: [{
				title: 'Финансовый словарь',
				redirect_url: '/finansoviy-slovar/',
				keywords: 'словарь'
			},
			{
				title: 'Написать статью в словарь',
				syntax: 'WIKI ADD',
				redirect_url: '/finansoviy-slovar/add/',
				auth_required: true,
				keywords: 'написать статью в словарь'
			},
			{
				title: 'Поиск в финсловаре',
				command: 'WIKI ',
				syntax: 'WIKI искомое',
				params: /^(.*)$/,
				ac: /^(.*)/i,
				act: 'wiki',
				url: '/finansoviy-slovar/$1'
			}
		]
	},
	'G': {
		title: 'Графики онлайн',
		variants: [{
				title: 'Графики любых инструментов',
				start_title: 'График',
				redirect_url: '/g/',
				keywords: 'график'
			},
			{
				title: 'Графики онлайн',
				command: 'G ',
				syntax: 'G ticker timeframe',
				params: /^([0-9a-z]+)(?:\s(\d+|H|D|W|M))?$/i,
				ac: /^([A-Z]+)/i,
				act: 'symbol',
				url: '/g/$1/$2/',
				tooltip: 'Введите тикер инструмента'
			}
		]
	},
	...
	'Q': {
		title: 'Котировки',
		variants: [{
				title: 'Котировки акций на МосБирже',
				redirect_url: '/q/shares/',
				keywords: 'акции котировки,котировки акций,курс акций'
			},
			{
				title: 'Перейти на форум акции',
				command: 'Q ',
				syntax: 'Q ticker-акции',
				params: /^(.*)$/,
				ac: /^(.*)/i,
				act: 'forum',
				url: '/forum/company_go/$1/'
			} // params: /^([0-9a-z]+)$/i,
		]
	},

	'OIL': {
		title: 'Нефть: график, запасы, новости',
		redirect_url: '/oil/',
		keywords: 'нефть'
	},
	'B': {
	...
	'TOP': {
		title: 'Лучше посты смартлаба',
		redirect_url: '/top/',
		keywords: 'лучшие посты,рейтинг постов'
	},
	'SGNL': {
		title: 'Лента торговых сигналов',
		redirect_url: '/blog/tradesignals/',
		keywords: 'лента сигналов,сигналы,торговые сигналы'
	},
	'TAGS': {
		title: 'Список всех тегов',
		redirect_url: '/tags/',
		keywords: 'теги все'
	},
	'CORP': {
		...
		variants: [{
				...
				keywords: 'брокеры блоги,корпоративные блоги'
			},
			{
				title: 'Создать корпоративный блог',
				syntax: 'CORP ADD',
				redirect_url: '/company/add/',
				keywords: 'корпоративный блог создать,создать корп блог'
			},
			{
				title: 'Корпоративный блог компании',
				command: 'CORP ',
				syntax: 'CORP название',
				params: /^(.*)$/,
				ac: /^(.*)$/i,
				act: 'company',
				url: '/company/$1/'
	...
	'$': {
		title: 'Форум компании',
		start_title: 'Акции компаний',
		command: '$',
		syntax: '$название или тикер',
		params: /^(.*)$/,
		ac: /^(.*)/i,
		act: 'forum$',
		url: '/forum/company_go/$1/',
		tooltip: 'Введите название компании',
		keywords: 'искать компанию,найти акцию компании'
	}, // hardcoded

	'FAV': {
		title: 'Избранное',
		variants: [{
				title: 'Ваши избранные посты',
				url: '/profile/$login/favourites/',
				auth_required: true,
				keywords: 'избранное мое,мое избранное'
			},
			{
				...
				tooltip: 'введите имя чтобы посмотреть избранное',
				keywords: 'избранное человека'
			}
		]
	},
	...
	'FRND': {
		title: 'Френдлента',
		variants: [{
				title: 'Ваша лента друзей',
				url: '/friends/$login/',
				auth_required: true,
				keywords: 'лента друзей,френдлента'
			},
			{
				title: 'Френдлента пользователя',
				command: 'FRND @',
				syntax: 'FRND @login',
				params: /^\@([0-9a-z\_\-]+)$/i,
				ac: /^\@(.*)$/i,
	...
	'LINKS': {
		title: 'Ссылки',
		variants: [{
				title: 'Ваши сохраненные ссылки',
				url: '/my/$login/tree/links/',
				auth_required: true,
				keywords: 'избранные ссылки,мои ссылки'
			},
			{
				title: 'Ссылки пользователя',
				command: 'LINKS @',
				syntax: 'LINKS @login',
				params: /^\@([0-9a-z\_\-]+)$/i,
				ac: /^\@(.*)$/i,
	...
	'CMT': {
		...
		variants: [{
				...
				url: '/my/$login/comment/',
				auth_required: true,
				keywords: 'комментарии мои,мои комментарии'
			},
			{
	...
	'TV': {
		title: 'Лента всех видео',
		redirect_url: '/smartlab-tv/',
		keywords: 'видео на смартлабе'
	},
	'POST': {
		title: 'Написать пост в свой блог',
		redirect_url: '/topic/add/',
		auth_required: true,
		keywords: 'написать пост,пост написать'
	},
	'ACC': {
		title: 'Статистика вашего счета',
		redirect_url: '/my-trading-account/',
		auth_required: true,
		keywords: 'мой счет,статистика счета'
	},
	'MAIL': {
		title: 'Личная переписка',
		auth_required: true,
		variants: [{
			...
			},
			{
				...
				command: 'MAIL TO @',
				syntax: 'MAIL TO @login',
				params: /^TO\s\@([0-9a-z\_\-]+)$/i,
				ac: /^TO\s\@(.*)$/i,
				func: 'mailto_func',
				auth_required: true
			}
	...
	'CCI': {
		title: 'Индексы доверия смартлаба',
		redirect_url: '/consumer-confidence-russia/'
	},
	'CHAT': {
		title: 'Секретный клуб',
		redirect_url: '/chat/',
		hidden: true
	},
	'CAT': {
		title: 'Каталог финансовых услуг',
		redirect_url: '/page/catalog/'
	},
	'ADV': {
		title: 'Размещение рекламы',
		redirect_url: 'https://smart-lab.ru/blog/mytrading/7053.php'
	},
	'MAIN': {
		title: 'Перекидывает на главную',
		redirect_url: '/'
	},
	'FINAM': {
		title: 'Сайт Финам',
		redirect_url: 'http://www.finam.ru/'
	},
	'MAP': {
		title: 'Тепловая карта рынка, форума и блогов',
		redirect_url: '/q/map/',
		keywords: 'акции карта рынка,карта рынка'
	},
	'FLOW': {
		title: 'Все блоги одним потоком',
		redirect_url: '/flow/',
		keywords: 'поток постов всех блогов'
	},
	'FXPF': {
		title: 'Графики-онлайн на Forexpf',
		redirect_url: 'http://www.forexpf.ru/chart/'
	},
	'NEWS': {
		title: 'Новости по российским акциям',
		variants: [{
				title: 'Все новости',
				redirect_url: '/news/',
				keywords: 'лента новостей,новости'
			},
			{
				title: 'Новости по Сбербанку',
				syntax: 'NEWS $SBER',
				redirect_url: '/forum/news/SBER/'
			},
			{
				title: 'Новости по любой компании',
				syntax: 'NEWS $TICKER',
				params: /^\$([0-9a-z\_\-]+)$/i,
				ac: /^\$(.*)$/i,
				act: 'forum',
				url: '/forum/$1/attached/'
			}
		]
	},
	'OFFTOP': {
		title: 'Оффтоп',
		variants: [{
				title: 'Посмотреть раздел ОФФТОП',
				redirect_url: '/blog/offtop/',
				keywords: 'оффтоп раздел'
			},
			{
				title: 'Написать пост не по теме смартлаба',
				syntax: 'OFFTOP ADD',
				redirect_url: '/topic/add/?blog_id=4107',
				keywords: 'написать в оффтоп,оффтоп написать',
				auth_required: true
			}
		]
	},
	'EVENTS': {
		title: 'Календарь тусовок',
		redirect_url: '/calendar/index/party/'
	},
	'SWITCH': {
		title: 'Переключить цветовую схему сайта',
		auth_required: true,
		variants: [{
				title: 'Стандартная тема',
				syntax: 'SWITCH BLACK',
				redirect_url: '/settings/theme/black',
				auth_required: true
			},
			{
				title: 'Белая тема',
				syntax: 'SWITCH WHITE',
				redirect_url: '/settings/theme/white',
				auth_required: true,
				keywords: 'сделать смартлаб белым'
			},
			{
				title: 'Полностью черная тема',
				syntax: 'SWITCH FULLBLACK',
				redirect_url: '/settings/theme/fullblack',
				auth_required: true,
				keywords: 'сделать смартлаб полностью черным'
			},
			{
				title: 'Оригинальная тема',
				syntax: 'SWITCH ORIGINAL',
				redirect_url: '/settings/theme/original',
				auth_required: true,
				keywords: 'сделать смартлаб в старой теме'
			}
		]
	},
	'LINK': {
		title: 'Переход по спец-ссылке',
		command: 'LINK ',
		syntax: 'LINK name',
		params: /^(.*)$/,
		url: '/console/link/$1/'
	...
	'EXIT': {
		title: 'Разлогиниться',
		redirect_url: '/login/exit/',
		auth_required: true
	},


	'STOCKS': {
		title: 'Форум по акциям',
		redirect_url: '/forum/',
		keywords: 'акции форум,форум акций'
	},
	'BONDS': {
		title: 'Форум по облигациям',
		redirect_url: '/bonds/',
		keywords: 'форум облигаций'
	},
	'FX': {
		title: 'Форум по Forex',
		redirect_url: '/forex/',
		keywords: 'форекс,форум forex'
	},
	'BANKS': {
		title: 'Форум по банкам',
		redirect_url: '/banki/',
		keywords: 'форум банков'
	},
	'OPT': {
		title: 'Форум по опционам',
		redirect_url: '/options/',
		keywords: 'опционы форум'
	},
	'ALGO': {
		title: 'Форум по торговым роботам',
		redirect_url: '/algotrading/',
		keywords: 'алготрейдинг форум,торговые роботы форум'
	},
	'BROK': {
		title: 'Рейтинг брокеров и отзывы',
		redirect_url: '/brokers-rating/',
		keywords: 'брокеры,выбрать брокера,рейтинг брокеров'
	},

	'ECOS': {
		title: 'Календарь событий по акциям',
		redirect_url: '/calendar/stocks/',
		keywords: 'акции календарь,календарь акций'
	},
	'FA': {
		title: 'Фундаментальный анализ акций',
		redirect_url: '/q/shares_fundamental/',
		keywords: 'акции фунд.анализ,мультипликаторы компаний,фундаментальный анализ акций'
	},
	'QBO': {
		title: 'Котировки облигаций',
		redirect_url: '/q/ofz/',
		keywords: 'бонды котировки,котировки ОФЗ,облигации ОФЗ,ОФЗ'
	},
	'BROKG': {
		title: 'Рейтинг глобальных брокеров',
		redirect_url: '/brokers-rating/global/',
		keywords: 'брокеры Глобальные'
	},
	'BROKR': {
		title: 'Рейтинг российских брокеров',
		redirect_url: '/brokers-rating/russia/',
		keywords: 'брокеры Россия'
	},
	'BROKF': {
		title: 'Рейтинг форекс брокеров',
		redirect_url: '/brokers-rating/forex/',
		keywords: 'брокеры Forex'
	},
	'ASK': {
		title: 'Задать вопрос на смартлабе',
		redirect_url: '/vopros/add/',
		keywords: 'вопрос задать,задать вопрос',
		auth_required: true
	},
	'ANSWER': {
		title: 'Лента всех вопросов',
		redirect_url: '/vopros/',
		keywords: 'вопросы лента,лента вопросов,ответы на вопросы'
	},
	'W': {
		title: 'Ваши портфели (portfolio)',
		auth_required: true,
		variants: [{
				title: 'Ваш портфели (portfolio)',
				command: 'W',
				redirect_url: '/q/portfolio',
				keywords: 'портфолио,мое портфолио,портфель,списки акций мои,portfolio',
				auth_required: true
			},
			{
				...
				command: 'W ADD',
				syntax: 'W ADD',
				func: 'new_portfolio_func',
				keywords: 'создать портфель,создать портфолио',
				auth_required: true
			},
		]
	},
	'DIV': {
		title: 'Таблица дивидендов и даты отсечек',
		redirect_url: '/dividends/',
		keywords: 'дивиденды,закрытие реестров дивиденды,календарь дивидендов,реестр дивидендов'
	},
	'DIVR': {
		title: 'Информация о поступивших дивидендах',
		redirect_url: '/forum/Поступление%20дивидендов/',
		keywords: 'дивиденды поступление,поступление дивидендов'
	},

	'QBE': {
		title: 'Котировки еврооблигаций',
		redirect_url: '/q/eurobonds/',
		keywords: 'еврооблигации,котировки еврооблигаций'
	},
	'REG': {
		title: 'Календарь отсечек по акциям',
		redirect_url: '/calendar/index/stocks_otsechka',
		keywords: 'закрытие реестров календарь'
	},
	'PICS': {
		title: 'Лента всех картинок',
		redirect_url: '/pics/',
		keywords: 'картинки,лента картинок'
	},
	'BR': {
		...
		variants: [
			...
			{
				title: 'Лента книжных рецензий',
				syntax: 'BR',
				redirect_url: '/blog/reviews/',
				keywords: 'книги рецензии,рецензии книг'
			},
			{
				title: 'Написать отзыв к книге',
				syntax: 'BR ADD',
				redirect_url: '/review/add/',
				keywords: 'написать рецензию на книгу,рецензия написать',
				auth_required: true
			}
		]
	},
	...
	'TOPB': {
		...
		redirect_url: '/books/top/',
		keywords: 'книги топ-100,рейтинг книг'
	},
	'CMTS': {
		title: 'Лента всех комментариев',
		redirect_url: '/comments/',
		keywords: 'комментарии все,лента комментариев'
	},
	'CMTME': {
		title: 'Комментарии к вашим постам',
		redirect_url: '/my/$login/comment-to-me/',
		auth_required: true,
		keywords: 'комментарии к моим постам'
	},
	'QA': {
		title: 'Котировки АДР',
		redirect_url: '/q/adr/',
		keywords: 'котировки АДР'
	},
	'QW': {
		title: 'Котировки мировых индексов и Forex',
		redirect_url: '/q/world-quotes/',
		keywords: 'котировки мировые,курсы валют'
	},
	'QBC': {
		title: 'Котировки корпоративных облигаций',
		redirect_url: '/q/bonds/',
		keywords: 'котировки облигаций корп,облигации корпоративные'
	},
	'QBS': {
		title: 'Котировки облигаций субъектов РФ',
		redirect_url: '/q/subfed/',
		keywords: 'котировки облигаций субфед,облигации субъектов'
	},
	'QR': {
		title: 'Котировки валютного рынка Мосбиржи',
		redirect_url: '/q/currencies/',
		keywords: 'котировки рубля,курсы рубля'
	},
	'QF': {
		title: 'Котировки срочного рынка Мосбиржи',
		redirect_url: '/q/futures/',
		keywords: 'котировки фьючерсов'
	},
	'FORUMS': {
		title: 'Все записи форумов подряд',
		redirect_url: '/forums/',
		keywords: 'лента форумов,форумы лента'
	},
	'READS': {
		title: 'Самые полезные статьи (выбор редактора)',
		redirect_url: '/tradingreads/',
		keywords: 'лучшие статьи за все время,нетленка,трейдинг лучшие статьи'
	},
	'PEOPLE': {
		title: 'Все пользователи смартлаба',
		redirect_url: '/people/',
		keywords: 'люди,рейтинг блогеров,трейдеры все'
	},
	'MOB': {
		title: 'Переключиться на мобильную версию',
		redirect_url: '/mobile/',
		keywords: 'мобильная версия'
	},
	'BL': {
		title: 'Ваш черный список',
		redirect_url: '/blacklist/',
		keywords: 'мой черный список,черный список,чс',
		auth_required: true
	},
	'FORUMMY': {
		title: 'Ваши сообщения на форумах',
		redirect_url: '/my/$login/comment-forum/',
		keywords: 'мои сообщения на форуме',
		auth_required: true
	},
	'BOOKSDONE': {
		title: 'Книги, которые вы прочли и оценили',
		redirect_url: '/books/reviewed_by/$login/',
		keywords: 'мои прочитанные книги',
		auth_required: true
	},
	'REPORTS': {
		title: 'Таблица последних отчетов по МСФО',
		redirect_url: '/q/shares_fundamental3?&type=MSFO',
		keywords: 'мсфо'
	},
	'SETPROFILE': {
		title: 'Настройки вашего профиля',
		redirect_url: '/settings/profile/',
		keywords: 'настройки профиля',
		auth_required: true
	},
	'SETTINGS': {
		title: 'Настройки ваших уведомлений',
		redirect_url: '/settings/notifications/',
		keywords: 'настройки уведомлений',
		auth_required: true
	},
	'QBI': {
		title: 'Индексы российских облигаций',
		redirect_url: '/q/index_bonds/',
		keywords: 'облигации индексы'
	},
	'ECOB': {
		title: 'Календарь событий по облигациям',
		redirect_url: '/calendar/bonds/',
		keywords: 'облигации календарь'
	},
	...
	'POLL': {
		...
		variants: [

			{
				...
				syntax: 'POLL ADD',
				redirect_url: '/question/add/',
				keywords: 'опрос создать',
				auth_required: true
			}
	...
	'REPLY': {
		...
		redirect_url: '/my/$login/reply-to-me/',
		keywords: 'ответы на мои комментарии',
		auth_required: true
	},
	'FREPLY': {
		title: 'Ответы мне на форуме',
		redirect_url: '/my/$login/reply-to-me-forum/',
		keywords: 'ответы мне на форуме',
		auth_required: true
	},
	'RULES': {
		title: 'Правила смартлаба',
		redirect_url: '/blog/mytrading/5161.php',
		keywords: 'правила смартлаба'
	},
	'FORUMERS': {
		title: 'Рейтинг участников форума',
		redirect_url: '/people/all/order_by_stocks/desc/',
		keywords: 'рейтинг форумчан'
	},
	'READERS': {
		title: 'Рейтинг читателей книг',
		redirect_url: '/people/all/order_by_books/desc/',
		keywords: 'рейтинг читателей'
	},
	'RAS': {
		title: 'Таблица последних отчетов по РСБУ',
		redirect_url: '/q/shares_fundamental3?&type=RSBU',
		keywords: 'рсбу'
	},
	'SOFT': {
		title: 'Форум софта для трейдинга',
		redirect_url: '/trading-software/',
		keywords: 'торговый софт'
	},
	...
	$(document).ready(function () {
		...
		if ($('.trades').length) {
			$.ajax({
				...
				if (symbols.length) {
					new TradingView.QuotesProvider({
						container_id: 'hif',
						symbols: symbols
					});
				};
...
(function (b) {
	function a() {
		...
		function d(g, h) {
			...
			var e = new jBox("Confirm", {
				id: "abuseDialog",
				title: "Пожаловаться на " + (h in f ? f[h] : " жизнь"),
				confirmButton: "ОК",
				cancelButton: "Отмена",
				content: c,
				confirm: function () {
					...
					b.post("/abuse/abuse_ajax/", k, function (l, m) {
						if (!l) {
							msgErrorBox.alert("Error", "Please try again later");
						}
						if (l.bStateError) {
							msgErrorBox.alert(l.sMsgTitle, l.sMsg);
						} else {
							msgNoticeBox.alert(l.sMsgTitle, l.sMsg);
						}
					});
				},
				onOpen: function () {
...
(function (a) {
	a(document).ready(function () {
		window.ban = function (h) {
			...
			var g = {
				...
				confirm: function () {
					...
					var l = f.find('input[name="time"]');
					l.each(function (o, n) {
						if (n.checked) {
							m.time = n.value;
						}
					});
					var j = f.find('input[name="time_value"]');
					if (j.length) {
						m.time_value = j.val();
					}
					var k = f.find('input[name="reason"]');
					k.each(function (o, n) {
						if (n.checked) {
							m.reason = n.value;
						}
					});
					var j = f.find('input[name="reason_value"]');
					if (j.length) {
						m.reason_value = j.val();
					}
					a.ajax({
						url: "/profile/ajaxwarning/",
						type: "POST",
						data: m
					}).then(function (n) {
						if (!n) {
				...
				onOpen: function () {
					var i = f.find('input[name="time"]');
					i.each(function (l, k) {
						k.checked = false;
					});
					i[0].checked = true;
					var j = f.find('input[name="reason"]');
					j.each(function (l, k) {
						k.checked = false;
					});
					j[10].checked = true;
			...
			var e = null;
			a.ajax({
				url: aRouter.profile + "ajaxwarning/",
				type: "GET",
				data: {
					userLogin: h,
					security_ls_key: LIVESTREET_SECURITY_KEY
				...
				if (!i.bStateError) {
					...
					a("body").append(a(i.sHtml));
					f = a("#user_ban_dialog");
					g.content = f;
					e = new jBox("Confirm", g);
					e.open();
				} else {
					msgErrorBox.alert(i.sMsgTitle, i.sMsg);
				}
			});
		};
		window.unban = window.unwarn = function (e) {
			...
			a.ajax({
				...
				if (g.bStateError) {
					...
					if (g.sMsgTitle) {
						msgNoticeBox.alert(g.sMsgTitle, g.sMsg);
						setTimeout(function () {
							document.location.reload();
						}, 1000);
					} else {
		...
		var c = a("#login_pn");
		if (c) {
			var d = a('<span title="IP адреса пользователя">(X)</span>').css({
				...
				var f = a("#login_ipn");
				if (f) {
					f.css("display", (f.css("display") == "block") ? "none" : "block");
				}
				return false;
			}).appendTo(c);
			var b = a('<span title="Забанить или предупредить юзера">(Б)</span>').css({
				display: "inline-block",
				verticalAlign: "middle",
				marginLeft: "0.2em",
				cursor: "pointer",
				...
				h.preventDefault();
				var g = document.getElement('link[rel="canonical"]');
				if (g) {
					var f = g.href.match(/\/profile\/([\da-z0-9\_\-]+)/i);
					if (f) {
						window.ban(f[1]);
					}
				}
...
function Summoner(textbox, url, sid) {
	...
	function summonerDropdown(parentAutocomplete) {
		...
		var cursorCoordinates = getCaretCoordinates(textbox.get(0), suggestState.from, {
			debug: false
		});
				...
				for (var i = p, j = i - 100;
					...
					if (htmlItem.value.charAt(i) == '@') {
						...
						suggest = {
							'opened': true,
							'from': i,
							'word': htmlItem.value.substring(i + 1, p)
						};
						break;
...
function commentsForm(commentBox, commentsRoot) {
	...
	this.placeCommentForm = function (options) {
		...
		if (params.scroll) {
			...
			if (commentBoxBottom > documetntScrolledBottom) $('body').animate({
				scrollTop: $('body').scrollTop() + commentBoxBottom - documetntScrolledBottom
			}, 500);
		...
		promise = promise.then(function () {
			...
			return $.ajax({
				...
				data: {
					...
					chatMode: commentBox.attr('chatmode') ? 1 : 0
				}
	...
	commentsRoot.on('click', 'a.reply, .edit, .abuse, .delete, .favorite, .th.up, .th.dn', function (e) {
		...
		if (el.hasClass('edit') || el.hasClass('reply')) {
			self.placeCommentForm.call(self, {
				node: commentNode,
				quote: el.hasClass('reply'),
				edit: el.hasClass('edit')
			});
			...
			promise.then(function () {
				if (btn.data('type') == 'reply') {
					promise = promise.then(function () {
						...
						return $.ajax({
							url: sUrl + '?JsHttpRequest=' + (new Date()).getTime() + '-form', // ебаное гавно с JsHttpRequest
							type: 'POST',
							data: {
								comment_text: textArea.val().replace(/\r\n|\r|\n/g, "<br />"),
								reply: commentId,
								cmt_target_id: targetId,
								security_ls_key: LIVESTREET_SECURITY_KEY
							}
						});
					...
					promise = promise.then(function () {
						return $.ajax({
							url: '/topic/ajax_edit_comment/?JsHttpRequest=' + (new Date()).getTime() + '-form', // ебаное гавно с JsHttpRequest
							type: 'POST',
							data: {
								comment_text: textArea.val().replace(/\r\n|\r|\n/g, "<br />"),
	...
	$('#comments-autouptate').on('click', function (e) {
		...
		$.ajax({
			...
			type: 'POST',
			data: {
				bRefreshComments: bAutoupdate ? 1 : 0,
				security_ls_key: LIVESTREET_SECURITY_KEY
			}
		}).then(function (result) {
			try {
...
$(document).ready(function () {
	...
	$('.graph_button').on('click', function (e) {
		...
		$.ajax({
			...
			type: 'POST',
			data: {
				bDisplayGraph: btn.hasClass('hidden') ? 1 : 0,
				security_ls_key: LIVESTREET_SECURITY_KEY
			}
		}).then(function (result) {
			try {
		...
		var moderatorsDialogParam = {
			...
			saveModerators: function () {
				...
				$.ajax({
					...
					type: 'POST',
					data: {
						id: self.options.articleId,
						moderators: aModerators,
						security_ls_key: LIVESTREET_SECURITY_KEY
					}
		...
		$('#trading_add').on('click', function (e) {
			...
			var addArticleDialogParam = {
				...
				_onCreated: function () {
					...
					jQuery('<button class="close"></button>').click(function () {
						this.options.cancel && this.options.cancel();
						this.close();
					}.bind(this)).appendTo(this.container.find('.jBox-title'));
				},
	...
	$('#more_disclosure_commented').on('click', function (e) {
		...
		$.ajax({
			...
			type: 'POST',
			data: {
				skip: $('li:not(.popCommented, .more)', '#disclosure_commented_block').length,
				security_ls_key: LIVESTREET_SECURITY_KEY
			}
	...
	$('#more_last_commented').on('click', function (e) {
		...
		$.ajax({
			...
			type: 'POST',
			data: {
				skip: $('li:not(.popCommented, .more)', '#last_commented_block').length,
				security_ls_key: LIVESTREET_SECURITY_KEY
			}
	...
	$('#more_last_commented2').on('click', function (e) {
		...
		$.ajax({
			...
			type: 'POST',
			data: {
				skip: $('.post', '#last_commented_block').length,
				security_ls_key: LIVESTREET_SECURITY_KEY
			}
				...
				var dialog = new jBox('Confirm', {
					...
					confirm: function () {
						var data = {
							...
							text: dialog.wrapper.find('#selectedText').val(),
							comment: dialog.wrapper.find('#commentText').val(),
							security_ls_key: LIVESTREET_SECURITY_KEY
						};
