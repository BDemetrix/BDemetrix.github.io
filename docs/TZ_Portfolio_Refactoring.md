...
- [ ] Отображается корректно на 320px, 768px, 1200px
...
- [ ] Весь контент переведён на английский (i18n.js содержит оба словаря)
- [ ] `<html lang>` меняется при переключении языка
- [ ] Все ссылки на проекты открываются в новой вкладке
- [ ] iframe или скриншоты для актуальных проектов (раздел 4.2)
- [ ] Блок «От вёрстки к архитектуре» содержит все 7 ссылок-тегов (раздел 4.3)
- [ ] Кнопка «Скачать резюме» скачивает PDF
- [ ] Кнопка «Написать в Telegram» ведёт на @Meaning_Driven
- [ ] Back-to-top появляется при скролле
- [ ] Scroll-анимации работают (_scr-item)
- [ ] PageSpeed Desktop ≥ 90, Mobile ≥ 85
- [ ] В консоли нет ошибок JS
- [ ] Schema.org Person валиден (проверить через schema.org/validator)
- [ ] Open Graph корректен (проверить через opengraph.xyz)
